...
void solve() {
    ...
    ll curSum = 0;
    for (auto &[bi, ai] : p) {
        if (bi > ai) curSum += bi - ai;
    }
    priority_queue<ll> small;
    priority_queue<ll, vll, greater<ll>> big;
    ll smallSum = 0;
    for (int i = n - 1; i >= 0; i--) {
        auto [bi, ai] = p[i];
        if (bi > ai)
            curSum -= bi - ai;
        if (small.size() == k - 1)
            ans = max(ans, curSum - smallSum - ai);
        if (!small.empty() && small.size() == k - 1 && small.top() > ai) {
            smallSum -= small.top();
            small.pop();
        }
        // cout << ai << " " << bi << " " << i << " " << ans << " " << smallSum << " " << curSum << endl;
        if (small.size() < k - 1) {
            small.push(ai);
            smallSum += ai;
        }

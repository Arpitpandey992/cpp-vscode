...
void solve() {
    ...
    for (ll i = n; i >= 0; i--) {
        for (ll ops = 0; ops <= k; ops++) {
            ...
            DP[i][ops] = a[i] + DP[i + 1][ops];
            ll curMin = a[i];
            for (ll take = 1; take <= ops && i + take <= n - 1; take++) {
                curMin = min(curMin, a[i + take]);
                DP[i][ops] = min(DP[i][ops], curMin * (take + 1) + DP[i + take + 1][ops - take]);
            }
        }
    }
    cout << DP[0][k] << endl;
    // DP -> i, operations left = at max 10!
}

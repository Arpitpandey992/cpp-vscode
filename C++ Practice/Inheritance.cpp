...
class Mint {
   ...
   public:
    ...
    template <typename T>
    Mint &operator^=(const T &other) {  // For Modular Exponentiation
...
class Creature {
   public:
    ...
    int legs;
    virtual void pri() {
        cout << "Within Creature\n";
    }
    Creature(string name, int legs) : name(name), legs(legs) {}
...
class Pokemon : public Creature {
   ...
   public:
    ...
    Pokemon(int legs) : Creature("Undefined", legs) {}
    void printName() {
        cout << name << " is a Pokemon" << endl;
    }
    void printLegs() {
...
void solve() {
    ...
    void *ptr = &x;
    double f = x / 2;
    double t = (*(double *)ptr) / 2;
    cout << f << " " << t << endl;
    vector<Creature *> c = {new Pokemon("pikachu", 2),
                            new Animal("whale", 0),
                            new Pokemon("Snorlax", 2)};
    for (auto &creature : c) {
        creature->printName();
        creature->printLegs();
    }

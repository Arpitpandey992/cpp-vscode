...
void delivery() {
    deliver.lock();
    onMyWay = true;
    cout << "Can Has Been Ordered!" << endl;
    sleep(20);
    for (int inc = 0; inc < m; inc++) {
        canCount++;
        sem_post(&cans);
    }
    c.notify_all();  // wake up all employees
    cout << "Crate Refilled!, Waking Everyone" << endl;
    cout << "Currently Sleeping\n";
    for (auto &x : sl) {
        cout << x << " ";
    }
    cout << endl;
...
void getCrate() {
    if (!onMyWay) {
        thread deliveryGuy(delivery);
        deliveryGuy.detach();
    }
}
void coke(int eno) {
    while (canCount == 0) {  // crate is Empty, sleep (after ordering the crate)
        getCrate();
        sl.insert(eno);
        // unique_lock<mutex> ul(deliver);
        // c.wait(ul);  // thread will wait here untill cans are refilled and thread is woken up again
    }
    sem_wait(&cans);
    sl.erase(eno);
    canCount--;
...
void empFunction(int eno) {
    while (true) {
        coke(eno);
        cout << "Employee " << eno << " is Working\n";
        cout << "CanCount = " << canCount << endl;
        sleep(rand() % 5);
    }
}

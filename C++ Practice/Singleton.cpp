...
class Singleton {
   public:
    ...
    int getRandomNumber(int l = 0, int r = INT_MAX) {
        cout << x << endl;
        return (rand() % (r - l)) + l;
    }
   ...
   private:
    int x;
    inline static Singleton *instance = NULL;
    Singleton() {}
    Singleton &operator=(Singleton &s2) = delete;
    Singleton(const Singleton &s2) = delete;
};

//  Singleton *Singleton::instance;

void callInstance() {
    cout << Singleton::getInstance().getRandomNumber() << endl;
}

void solve() {
    vector<thread> t(4);
    for (auto &th : t) {
        th = thread(callInstance);
    }
    for (auto &th : t)
        th.join();
}

...
void solve() {
    ...
    vector<ll> ans;
    fo(i, m) {
        auto &worker = p[i], time = t[i];
        if (q[worker].size())
            q[worker].push_back(time + q[worker].back());
        else
            q[worker].push_back(time);
        ll pushed = q[worker].back();
        auto it = lower_bound(ans.begin(), ans.end(), pushed) - ans.begin();
        curSum += pushed;
        cout << curSum << endl;
    }
}

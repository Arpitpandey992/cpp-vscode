    ...
    test {
        ...
        for (auto it = s.begin(); it != s.end(); it++) {
            ...
            if (ta != tb) {
                ...
                if (tb > ta)
                    mn.pb(make_pair(x, abs(tb - ta)));
                else
                    mp.pb(make_pair(x, abs(tb - ta)));
            }
        }
        ll ans = 0;
        if (flag) {
            ll itn = mn.size() - 1;
            for (ll itp = 0; itp < mp.size();) {
                ll x = mp[itp].ss, y = mn[itn].ss;
                ll a = mp[itp].ff, b = mn[itn].ff;
                if (x == y) {
                    if (smallc * 2 < min(a, b))
                        ans += smallc * x;
                    else

...
void solve2() {
    ...
    cin >> n;
    vector<string> a(n);
    map<char, vector<int>> full;
    map<char, int> fst, last;
    set<char> mid, there;
    fo(i, n) cin >> a[i];
    fo(i, n) {
        string &s = a[i];
        ...
        last[s[s.size() - 1]] = i;
        fok(j, 1, s.size() - 1) {
            if (s[j] == s[0] || s[j] == s[s.size() - 1]) continue;
            if (CHK(there, s[j])) {
                cout << "IMPOSSIBLE";
                return;
            }
        }
        for (auto &x : s) there.insert(x);
    }
    set<char> done;
    char cur = '?';
    string ans;
    while (1) {
        if (cur == '?') {
            bool flag = true;
            for (char c = 'A'; c <= 'Z'; c++) {
                if (CHK(done, c)) continue;
                if (!CHK(last, c) && (CHK(fst, c) || CHK(full, c))) {
                    flag = false;
                    done.insert(c);
                    if (CHK(full, c)) {
                        for (auto &x : full[c]) {
...
void solve1(int n, vector<string> &a) {
    map<char, vector<int>> full;
    map<char, int> fst, last;
    set<char> mid, there;
    vi v;

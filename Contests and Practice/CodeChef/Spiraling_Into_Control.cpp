...
int dy[] = {0, 1, 0, -1};
vector<pii> ans;
void fun(int x, int y, vector<vector<int>> &a, vector<vector<int>> &sp, int &n) {
    if (x == n / 2 && y == n / 2) return;
    fo(k, 4) {
        int nx = x + dx[k], ny = y + dy[k];
        if (nx < 0 || nx >= n || ny < 0 || ny >= n || sp[nx][ny] != sp[x][y] - 1 || a[nx][ny] < a[x][y]) continue;
        if (a[nx][ny] != a[x][y] + 1) {
            ans.pb({a[x][y], a[nx][ny]});
        }
        fun(nx, ny, a, sp, n);
        return;
    }
...
void solve() {
    ...
    while (q.size()) {
        ...
        fo(k, 4) {
            int nx = x + dx[k], ny = y + dy[k];
            if (nx < 0 || nx >= n || ny < 0 || ny >= n || sp[nx][ny] != -1 || a[nx][ny] > a[x][y]) continue;
            q.push({nx, ny, dis + 1});
            sp[nx][ny] = dis + 1;
        }
    }

...
void draw_peri(int r, int R, vvi &a) {
    for (int x = -r; x <= r; x++) {
        double Y = round(sqrt(r * r - x * x));
        if (isint(Y * 2))
            Y = trunc(Y);
        else
            Y = round(Y);
...
void draw_full(int R, vvi &a) {
    for (int x = -R; x <= R; x++) {
        for (int y = -R; y <= R; y++) {
            double Y = (sqrt(x * x + y * y));
            if (isint(Y * 2))
                Y = trunc(Y);
            else
                Y = round(Y);
...
void solve() {
    int R;
    cin >> R;
    vvi a(2 * R + 2 * k, vi(2 * R + 2 * k, 0));
    vvi b(2 * R + 2 * k, vi(2 * R + 2 * k, 0));
    fo(i, R + 1) {
        draw_peri(i, R, b);
    }
    draw_full(R, a);

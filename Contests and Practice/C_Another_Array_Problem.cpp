...
void solve() {
    ...
    // n==3 and max at middle
    // ll ans = 0;
    // dp(a, ans);
    // cout << ans << endl;
    ll ans = a[0] + a[1] + a[2];
    ans = max(ans, abs(a[0] - a[2]) * 3);
    ans = max(ans, a[0] + abs(a[1] - a[2]) * 2);
    ans = max(ans, a[2] + abs(a[1] - a[0]) * 2);
    ans = max(ans, abs(a[1] - a[0]) + abs(abs(a[1] - a[0]) - a[2]) * 2);
    ans = max(ans, abs(a[1] - a[2]) + abs(abs(a[1] - a[2]) - a[0]) * 2);
    ans = max(ans, abs(abs(a[1] - a[2]) - a[0]) * 3);
    ans = max(ans, abs(abs(a[1] - a[0]) - a[2]) * 3);

...
/*------------------------------------------END OF TEMPLATE-------------------------------------------*/
bool can(ll n, ll l, ll s, bool exact = false) {
    ll req = ceil(2ll * s + l * l - l, 2ll * l);
    if (exact) {
        if (req == s)
            return true;
        return false;
    }
    for (ll x = max(l, req); x <= n; x++) {
        if (x - l < 0) continue;
        ll sum = (x * (x + 1)) / 2ll - (((x - l) * (x - l + 1))) / 2ll;
        if (sum < s)
            continue;
        ll req = sum - s;
        if (req >= x - l + 1 && req <= x) {

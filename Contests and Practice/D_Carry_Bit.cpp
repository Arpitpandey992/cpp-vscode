...
void solve() {
    ...
    for (int div = 1;; div++) {
        if (div % 2 == 1) {
            // 111111000000111110000...
            int minReq = (div - 1) / 2;
            if (n - k < minReq) break;
            // to partition n 1s into (div+1)/2 boxes -> n-1Ck-1
            int boxes = (div + 1) / 2;
            Mint ways = ncr(k - 1, boxes - 1);
            int gaps = boxes;
            Mint gapWays = ncr(zeros - 1, gaps - 1);
            ways *= gapWays;
            // each has div ups and downs -> each up and down
            // converts one 3 into one 1
            ll upsDowns = div;
            Mint cur = Mint(3) ^ (n - 1 - upsDowns);
            ans += ways * cur;
        } else {
            // 0000011111100000011100111100...
            int minReq = div / 2;
            if (n - k < minReq) break;
            int boxes = div / 2;
            Mint ways = ncr(k - 1, boxes - 1);

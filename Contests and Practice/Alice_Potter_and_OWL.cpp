...
void solve() {
    ...
    auto solveQuery = [&](int l, int r) {
        // if (r - l + 1 > 180) {
        //     vector<int> pref(181, -1);
        //     int cur = 0;
        //     pref[0] = l - 1;
        //     for (int i = l; i <= r; i++) {
        //         cur = (cur + a[i]) % M;
        //         if (pref[cur] != -1) {
        //             cout << pref[cur] + 1 << " " << i + 1 << endl;
        //             return;
        //         }
        //         pref[cur] = i;
        //     }
        // }
        int ans = INT_MAX, ansl = -1, ansr = -1;
        map<int, int> pref;
        int cur = 0;
        pref[0] = l - 1;
        for (int i = l; i <= r; i++) {
            cur = (cur + a[i]) % M;
            {
                auto &[v, where] = *pref.begin();
                if (v <= cur) {
                    int curAns = M - (cur - v);
                    if (ans > curAns && curAns <= 90) {
                        ans = curAns;
                        ansl = where + 1, ansr = i;
                    }
                }

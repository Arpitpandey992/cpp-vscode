...
int calc(int i, int j) {
    vpii pts = {{0, 0}, {n - 1, 0}, {0, m - 1}, {n - 1, m - 1}};
    int ans = 0;
    for (auto [x, y] : pts) {
...
void solve() {
    cin >> n >> m;
    double cx = (n - 1) / 2, cy = (m - 1) / 2;
    auto lbd = [&cx, &cy](auto &p1, auto &p2) {
        double x1 = p1.ff, y1 = p1.ss, x2 = p2.ff, y2 = p2.ss;
        return abs(cx - x1) + abs(cy - y1) > abs(cx - x2) + abs(cy - y2);
    };
    vector<tuple<int, int, int>> a;
    priority_queue<pii, vpii, decltype(lbd)> q(lbd);
    fo(i, n)
        fo(j, m) q.push({i, j});
    while (q.size()) {
        auto [x, y] = q.top();

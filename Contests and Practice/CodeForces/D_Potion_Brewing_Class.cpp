...
#pragma endregion
//-------------------------------------------------------------------------------//
Mint lcm(Mint x, Mint y) {
    Mint g = __gcd((ll)x, (ll)y);
    return (x * y) / (g);
}
const int N = 2e5 + 1;
map<int, int> facs[N + 5];
vector<int> den(N + 1, 0), f(N + 1, 0);
void dfs(int i, int par, vector<tuple<ll, int, int>> adj[]) {
    for (auto &[x, n, d] : adj[i]) {
        if (i == par) continue;
        auto &num1 = facs[n];
        for (auto &[key, val] : num1) {
            den[key] += val;
            f[key] = max(f[key], den[key]);
        }
        dfs(x, i, adj);
        for (auto &[key, val] : num1)
            den[key] -= val;
...
void dfs2(int i, int par, vector<tuple<ll, int, int>> adj[]) {
    for (auto &[x, n, d] : adj[i]) {
        if (i == par) continue;
        auto &den1 = facs[d];
        for (auto &[key, val] : den1) {
            den[key] += val;
            f[key] = max(f[key], den[key]);
        }
        dfs(x, i, adj);
...
void solve() {
    ll n;
    cin >> n;
    vector<tuple<ll, int, int>> adj[n];
    fo(i, n - 1) {
        ll u, v, x, y;
        cin >> u >> v >> x >> y;
        ll g = __gcd(x, y);
        adj[u - 1].pb({v - 1, x / g, y / g});
        adj[v - 1].pb({u - 1, y / g, x / g});
    }
    f[1] = 1;
    den[1] = 1;
    dfs(0, -1, adj);

...
vector<int> secondNextGreatest(vector<int>& a, int x) {
    int n = a.size();
    vector<int> nge(n, n);
    stack<int> s;
    for (int i = 0; i < n; i++) {
        while (s.size() && a[s.top()] + x < a[i]) {
            nge[s.top()] = i;
            s.pop();
        }
        s.push(i);
...
int maximumFriendshipPower(vector<pair<int, int>> edges, vector<int> b) {
    int n = b.size();
    DSU dsu(n);
    for (auto& [u, v] : edges) {
        dsu.uni(u, v);
...
void solve() {
    cout << maximumFriendshipPower({{0, 2}, {1, 3}}, {6, 2, 3, 1}) << endl;
}

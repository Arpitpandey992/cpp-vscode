...
void solve() {
    ...
    vector<int> p(n);
    iota(all(p), 0);
    sort(all(p), [&](auto &i1, auto &i2) {
        return a[i1] > a[i2];
    });
    priority_queue<pair<int, int>> q;
    vector<pair<int, int>> ans;
    int req = 0, mon = 0;
    for (auto &[tableSize, tableIndex] : t) {
        while (p.size() && a[p.back()].first <= tableSize) {
            q.push({a[p.back()].second, p.back()});
            p.pop_back();
        }
        if (q.empty()) continue;
        ans.push_back({q.top().ss, tableIndex});
        req++;
        mon += q.top().ff;
        q.pop();
    }

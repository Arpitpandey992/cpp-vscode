...
void solve() {
    ...
    auto dfs = [&](int i, int k, auto &dfs) -> int {
        
        int sum = b[i] * k;
        int siz = adj[i].size();
        if (siz == 0) return sum;

        if (DP.contains(mp(i, k)))
            return DP[mp(i, k)];
        int mn = k / siz;
        int extra = k % siz;
        if (extra == 0) {
            ...
            vector<vector<int>> a;
            for (auto &x : adj[i]) {
                auto five = dfs(x, mn + 1, dfs), four = dfs(x, mn, dfs);
                a.push_back({five - four, five, four});
            }
            Sort(a);
            while (a.size()) {
                auto v = a.back();
                int ct = v[0], five = v[1], four = v[2];
                a.pop_back();
                if (extra > 0) {
                    sum += five;
                    extra--;
        ...
        return DP[mp(i, k)] = sum;
    };
    cout << dfs(0ll, k, dfs) << endl;
}
#undef int
int main() {
#ifdef ONLINE_JUDGE

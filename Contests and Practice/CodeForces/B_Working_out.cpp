...
void solve() {
    ...
    fo(i, n) {
        fo(j, m) {
            int op1 = (i > 0 ? DP1[i - 1][j] : 0);
            int op2 = (j > 0 ? DP1[i][j - 1] : 0);
            DP1[i][j] = a[i][j] + max(op1, op2);
        }
    }
    for (int i = n - 1; i >= 0; i--) {
        for (int j = 0; j < m; j++) {
            int op1 = (i < n - 1 ? DP2[i + 1][j] : 0);
            int op2 = (j > 0 ? DP2[i][j - 1] : 0);
            DP2[i][j] = a[i][j] + max(op1, op2);
        }
    }
    for (int i = n - 1; i >= 0; i--) {
        for (int j = m - 1; j >= 0; j--) {
            int op1 = (i < n - 1 ? DP3[i + 1][j] : 0);
            int op2 = (j < m - 1 ? DP3[i][j + 1] : 0);
            DP3[i][j] = a[i][j] + max(op1, op2);
        }
    }
    fo(i, n) {
        for (int j = m - 1; j >= 0; j--) {
            int op1 = (i > 0 ? DP4[i - 1][j] : 0);
            int op2 = (j < m - 1 ? DP4[i][j + 1] : 0);
            DP4[i][j] = a[i][j] + max(op1, op2);
        }
    }
    int ans = 0;
    for (int i = 1; i < n - 1; i++) {
        for (int j = 1; j < m - 1; j++) {
            int op1 = DP1[i][j - 1] + DP3[i][j + 1] + DP2[i + 1][j] + DP4[i - 1][j];
            int op2 = DP1[i - 1][j] + DP3[i + 1][j] + DP2[i][j - 1] + DP4[i][j + 1];

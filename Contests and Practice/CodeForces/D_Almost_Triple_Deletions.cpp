...
void solve() {
    ...
    vi a(n);
    fo(i, n) cin >> a[i];
    // dp[i]->denotes the maximum length containing last element as a[i].
    // dp[i] = dp[j]+1 ; a[i]=a[j] && j+1....i-1 can be deleted && dp[j]!=0
    // can be deleted if max freq element occurs less than half of length and length % 2==0
    vvi mxfreq(n, vi(n));
    ...
    for (int i = 0; i < n; i++) {
        ...
        for (int j = i; j < n; j++) {
            freq[a[j]]++;
            cur = max(cur, freq[a[j]]);
            mxfreq[i][j] = cur;
        }
        for (int j = i; j < n; j++)
            freq[a[j]] = 0;
    }
    auto canDel = [&mxfreq](int i, int j) {
        if (i > j) return true;
        int len = j - i + 1;
        if (len & 1 || mxfreq[i][j] * 2 > len) return false;
        return true;
    };
    vector<int> DP(n, 0);
    fo(i, n) {
        if (canDel(0, i - 1)) DP[i] = 1;
        for (int j = i - 1; j >= 0; j--) {
            if (a[j] == a[i] && canDel(j + 1, i - 1) && DP[j] != 0)
                DP[i] = max(DP[i], 1 + DP[j]);
        }
    }
    int ans = 0;
    for (int i = 0; i < n; i++) {
        if (DP[i] != 0 && canDel(i + 1, n - 1))
            ans = max(ans, DP[i]);
    }
    cout << ans << endl;
}

...
vi dsu;
int fin(int i) {
    return dsu[i] = (dsu[i] == i ? i : fin(dsu[i]));
}
void solve() {
    cin >> n >> m >> k;
    c.resize(k);
    ...
    fo(i, m) {
        ...
        dis[par[u - 1]][par[v - 1]] = min(dis[par[u - 1]][par[v - 1]], x);
        dis[par[v - 1]][par[u - 1]] = min(dis[par[v - 1]][par[u - 1]], x);
    }

    fo(i, k) {
        int t = fin(fam[i].ff);
        fok(j, fam[i].ff, fam[i].ss) {
            if (fin(j) != t) {
                cout << "No\n";
                return;
            }

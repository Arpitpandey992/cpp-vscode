...
int dp(int j, int c, vector<int> &lps, vector<vector<int>> &DP, string &s) {
    if (j == 0) return 0;
    if (char(c + 'a') == s[j]) return j;
    if (DP[j][c] != -1) return DP[j][c];
    return DP[j][c] = dp(lps[j - 1], c, lps, DP, s);
}
...
void solve() {
    ...
    while (q--) {
        ...
        int ii = i, jj = j;
        while (ii - i < t.size()) {
            if (t[ii - i] == (jj < s.size() ? s[jj] : t[jj - i])) {
                lps[ii++] = ++jj;
            } else if (jj != 0) {
                if (jj >= s.size()) {
                    jj = lps[jj - 1];

...
pair<int, int> dfs(int i, vector<vector<int>> &adj) {
    if (adj[i].size() == 0) return {1, 1};
    // if (adj[i].size() == 1) {
    //     auto [len, linear] = dfs(adj[i][0], adj);
    //     return {max(len, linear + 1), linear + 1};  // i can extend the linear length
    // }

    int bestLen = 0, linLen = 1;
    for (auto &x : adj[i]) {
        auto [len, linear] = dfs(x, adj);
        linLen = max(linLen, linear + 1);
        bestLen += len;
    }
    bestLen = max(bestLen, linLen);
    return {bestLen, linLen};
}

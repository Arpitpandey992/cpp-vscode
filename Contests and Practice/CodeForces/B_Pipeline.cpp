...
void solve() {
    ...
    // coins ranging from 1,2,.........,k
    // need to make n using minimum denominations, cant use DP
    // n - [sum from mid.......k] < mid -> find last point where this happens
    ll totSum = (k * (k + 1)) / 2;
    if (totSum < n) {
        cout << -1 << endl;
        return;
    }
    auto canMake = [&](ll mid) -> bool {
        ll len = k - mid + 1;
        ll sum = ((mid + k) * len) / 2;  // sum = (a+l)*n/2
        if (n - sum < mid) return true;
        return false;
    };
    ll l = 1, r = k + 1;
    while (l < r) {
    ...
    l--;  // need to use pipes from l-1, l, ......, k-1, k
    ll len = k - l + 1;
    ll sum = ((l + k) * len) / 2;
    ll left = n - sum;
    if (left > 0)

...
ll calcTransition(ll x, char op, ll k) {
    if (op == '+') return x + k;
    if (op == '-') return x - k;
    if (op == '*') return x * k;
    return floor(x, k);
...
void solve() {
    ...
    sy--;
    ll DP[m+1][n+1][n+1];
    vector<pair<char, ll>> opk;
    vector<pair<ll, ll>> del = {{-1, 0}, {0, 1}, {1, 0}, {0, -1}};
    fo(i, 4) {
        char c;
        ll x;
        cin >> c >> x;
    ...
    for (ll moves = 0; moves <= m; moves++) {
        for (ll i = 0; i < n; i++) {
            for (ll j = 0; j < n; j++) {
                if (moves == 0) {
                    DP[moves][i][j] = (i == sx && j == sy) ? 0ll : -INFLL/10;
                    continue;
                }
                DP[moves][i][j] = DP[moves - 1][i][j];
                fo(dir, 4) {
                    auto &[dx, dy] = del[dir];
                    auto &[op, k] = opk[oppDir(dir)];
                    ll nx = i + dx, ny = j + dy;
                    if (nx < 0 || nx >= n || ny < 0 || ny >= n)
                        continue;
                    ll prev = DP[moves - 1][nx][ny];
                    DP[moves][i][j] = max(DP[moves][i][j], calcTransition(prev, op, k));
                }
            }
        }
    }
...
// DP[i][j][m]->maximum coins i can collect if i am at i,j and i have m moves left.

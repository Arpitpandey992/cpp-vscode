...
void buildst(int node, int sl, int sr, bool flag) {
    if (sl == sr) {
        st[node] = a[sl];
        return;
    }
    if (sl > sr) return;
    int mid = (sl + sr) / 2;
    buildst(node * 2 + 1, sl, mid, !flag);
    buildst(node * 2 + 2, mid + 1, sr, !flag);
    if (flag) {
...
void update(int node, int sl, int sr, int i, bool flag) {
    if (sl > i || sr < i) return;
    if (sl == sr) {
        st[node] = a[i];
        return;
    ...
    int mid = (sl + sr) / 2;
    update(node * 2 + 1, sl, mid, i, !flag);
    update(node * 2 + 2, mid + 1, sr, i, !flag);
    if (flag) {
        st[node] = st[node * 2 + 1] | st[node * 2 + 2];
    } else
        st[node] = st[node * 2 + 1] ^ st[node * 2 + 2];
...
void solve() {
    ...
    buildst(0, 0, n - 1, flag);
    fo(i, m) {
        int p, b;
        cin >> p >> b;
        p--;
        a[p] = b;
        update(0, 0, n - 1, p, flag);
        cout << st[0] << endl;
    }
}

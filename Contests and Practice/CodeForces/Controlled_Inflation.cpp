...
pll fun(ll cur, vll &a) {
    ll mx = a[a.size() - 1], mn = a[0];
    ll val1 = abs(mx - cur) + abs(mx - mn), val2 = abs(cur - mn) + abs(mx - mn);
    return {val1, val2};
}
...
void solve() {
    ...
    for (int i = n - 1; i >= 0; i--) {
        ...
        pll pp1 = fun(vmin, a[i]);
        pll pp2 = fun(vmax, a[i]);
        DP[i][0] = min(pp1.ff + (i < n - 1 ? DP[i + 1][0] : 0),
                       pp1.ss + (i < n - 1 ? DP[i + 1][1] : 0));
        DP[i][1] = min(pp2.ff + (i < n - 1 ? DP[i + 1][0] : 0),
                       pp2.ss + (i < n - 1 ? DP[i + 1][1] : 0));
    }
    cout << DP[0][0] << endl;
}

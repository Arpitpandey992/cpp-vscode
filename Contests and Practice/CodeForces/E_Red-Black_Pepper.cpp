...
void solve() {
    ...
    cin >> n;
    vpll a(n);
    fo(i, n) cin >> a[i].ff >> a[i].ss;
    sort(all(a), [](auto &p1, auto &p2) {
        if (p1.ff - p1.ss != p2.ff - p2.ss) return p1.ff - p1.ss > p2.ff - p2.ss;
        return p1.ff > p2.ff;
    });
    vll val(n + 1);  // max val i can get if i choose i red peppers
    vll pref(n), suff(n);
    fo(i, n) pref[i] = a[i].ff + (i > 0 ? pref[i - 1] : 0);
    for (int i = n - 1; i >= 0; i--) suff[i] = a[i].ss + (i < n - 1 ? suff[i + 1] : 0);
    for (int i = 0; i <= n; i++) {
        val[i] = (i > 0 ? pref[i - 1] : 0) + (i < n ? suff[i] : 0);
    }
    ll m;
    cin >> m;

...
#define modadd(x, y) (((x % M) + (y % M)) % M)  //modified for this Q
#define modsub(x, y) (((((x % M) - (y % M)) % M) + M) % M)
#define modmul(x, y) (((((x % M) * (y % M)) % M) + M) % M)
/* #region   */
#define endl '\n'
#define nl cout << '\n'
...
int modadd2(int a, int b, int m = M) {
    a = a % m;
    b = b % m;
    return (((a + b) % m) + m) % m;
}
ll modmul2(ll a, ll b, ll m = M) {
    a = a % m;
    b = b % m;
    return (((a * b) % m) + m) % m;
}
ll modsub2(ll a, ll b, ll m = M) {
    a = a % m;
    b = b % m;
    return (((a - b) % m) + m) % m;
}
ll moddiv(ll a, ll b, ll m = M)  //m must be prime
{
    a = a % m;
    b = b % m;
    return (modmul2(a, modinvprime(b, m), m) + m) % m;
}
/* #endregion */
//--------------------------------------------------------------------//
int DP[(int)(1e7 + 1)][4];
int main() {
    ...
    DP[1][3] = 0;
    fok(i, 2, n + 1) {
        DP[i][0] = modadd(DP[i - 1][1], modadd(DP[i - 1][2], DP[i - 1][3]));
        DP[i][2] = modadd(DP[i - 1][1], modadd(DP[i - 1][0], DP[i - 1][3]));
        DP[i][1] = modadd(DP[i - 1][2], modadd(DP[i - 1][0], DP[i - 1][3]));
        DP[i][3] = modadd(DP[i - 1][2], modadd(DP[i - 1][0], DP[i - 1][1]));
    }
    cout << DP[n][3] << endl;
}
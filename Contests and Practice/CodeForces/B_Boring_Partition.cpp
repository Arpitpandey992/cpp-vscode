...
void solve() {
    ...
    Sort(a);
    int ans = (a[n - 1].ff + a[n - 2].ff - a[1].ff - a[0].ff);
    int part = 0;
    for (int i = 2; i < n - 1; i++) {
        int mn = a[0].ff + a[1].ff;
        int mx = max(a[i - 1].ff + a[n - 1].ff + h, a[n - 1].ff + a[n - 2].ff);
        if (abs(mx - mn) < ans) {
            ans = abs(mx - mn);
            part = i;
        }
    ...
    if (n >= 3) {
        int mn = min(a[0].ff + a[1].ff + h, a[1].ff + a[2].ff);
        int mx = max(a[0].ff + a[n - 1].ff + h, a[n - 1].ff + a[n - 2].ff);
        if (abs(mx - mn) < ans) {
            ans = abs(mx - mn);
            part = 1;
        }

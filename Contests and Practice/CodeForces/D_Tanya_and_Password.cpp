...
string printCircuit(map<string, vector<string>> &adj) {
    unordered_map<string, int> edge_count;
    string curr_v;
    for (auto &p : adj) {
        curr_v = p.ff;
        edge_count[p.ff] = p.ss.size();
    }
...
vector<string> circuit;
void euler(string st, map<string, vector<string>> &adj) {
    auto &v = adj[st];
    while (v.size()) {
        auto t = v.back();
        v.pop_back();
...
void solve() {
    ...
    fo(i, n) {
        string t;
        cin >> t;
        string u = t.substr(0, 2), v = t.substr(1, 2);
        adj[u].pb(v);
        in[v]++;
        out[u]++;
        vert.insert(u);

...
void solve() {
    ...
    for (auto &[x, y] : X) Sort(y);
    for (auto &[x, y] : Y) Sort(y);
    ...
    while (q.size()) {
        ...
        auto &vx = X[x], &vy = Y[y];
        auto i1 = ub(vx, y) - vx.begin();
        auto i2 = i1 - 1;
        if (i1 < vx.size() && !CHK(vis, mp(x, vx[i1] - 1))) {
            vis.insert({x, vx[i1] - 1});
            q.push({x, vx[i1] - 1, dis + 1});
        }
        if (i2 >= 0 && !CHK(vis, mp(x, vx[i2] + 1))) {
            vis.insert({x, vx[i2] + 1});
            q.push({x, vx[i2] + 1, dis + 1});
        }
        auto j1 = ub(vy, x) - vy.begin();
        auto j2 = j1 - 1;
        if (j1 < vy.size() && !CHK(vis, mp(vy[j1] - 1, y))) {
            vis.insert({vy[j1] - 1, y});
            q.push({vy[j1] - 1, y, dis + 1});
        }
        if (j2 >= 0 && !CHK(vis, mp(vy[j2] + 1, y))) {
            vis.insert({vy[j2] + 1, y});
            q.push({vy[j2] + 1, y, dis + 1});
        }
    }
    cout << -1 << endl;
}

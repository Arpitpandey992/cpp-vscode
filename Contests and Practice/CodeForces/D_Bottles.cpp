...
void solve() {
    ...
    fo(i, n) cin >> a[i];
    fo(i, n) cin >> b[i];
    map<pair<int, int>, pll> DP;
    auto dp = [&](int i, int cap, auto &dp) -> pair<ll, ll> {
        if (i == n) {
            if (cap >= 0) return {0, 0};
            return {INF, INF};
        }
        if (DP.find(mp(i, cap)) != DP.end())
            return DP[mp(i, cap)];
        auto a1 = dp(i + 1, cap + b[i] - a[i], dp);
        a1.ff++;
        auto a2 = dp(i + 1, cap - a[i], dp);
        a2.ss += a[i];
        return DP[mp(i, cap)] = min(a1, a2);
    };
    auto ans = dp(0, 0, dp);

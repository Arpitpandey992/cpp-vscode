...
int st[4 * MAXN];
int buildst(int sl, int sr, int node, vector<int> &a) {
    if (sl > sr) return N;
    if (sl == sr) return st[node] = sl;
    int mid = (sl + sr) / 2;
    int lef = buildst(sl, mid, node * 2 + 1, a);
    int rig = buildst(mid + 1, sr, node * 2 + 2, a);
    if (a[lef] <= a[rig])
        return st[node] = lef;
    return st[node] = rig;
}
int mn(int l, int r, int sl, int sr, int node, vi &a) {
    if (sl >= l && sr <= r)
        return st[node];
    if (sr < l || sl > r)
        return N;
    int mid = (sl + sr) / 2;
    int lef = mn(l, r, sl, mid, node * 2 + 1, a);
    int rig = mn(l, r, mid + 1, sr, node * 2 + 2, a);
    if (a[lef] <= a[rig])
        return lef;
    return rig;
}

int mn2(int l, int r, vi &a) {
    int ans = l;
    for (int i = l; i <= r; i++) {
        if (a[ans] > a[i])
            ans = i;
...
void fun(int l, int r, vi &a, vi &lef, vi &rig) {
    if (l > r) return;
    // auto x = mn(l, r, 0, N - 1, 0, a);
    auto x = mn2(l, r, a);
    int i = lef[x] + 1, j = rig[x] - 1;
    if (j - i == ans)
        anss.pb(i);
    else if (j - i > ans) {
        ans = j - i;
...
void solve() {
    ...
    // fun(0, n - 1, a, lef, rig);
    Sort(anss);
    auto it = unique(all(anss));
    anss.resize(it - anss.begin());

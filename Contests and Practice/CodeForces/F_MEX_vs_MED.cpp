...
void solve() {
    ...
    for (int len = 1; len <= n; len++) {
        ...
        int wiggle = len - (curr - curl + 1);
        if (wiggle >= 0) {
            int lef = max(0, curl - wiggle);
            int rig = min(n - 1, curr + wiggle);
            int lefNext = rig - len + 1;
            ans += max(0, min(lefNext, curl) - lef + 1);
        }
    }
    cout << ans << endl;
}

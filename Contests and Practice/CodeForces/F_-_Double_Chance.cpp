...
#define T ll  // change if needed
   ...
   public:
    ...
    void update(int i, T x) {
        i += siz;
        st[i] = x;  // CAUTION -> Assignment operation, not addition
        i >>= 1;
        while (i > 0) {
...
void solve() {
    ...
    int mx = 2e5 + 1;
    SegTree st(mx + 1);
    // cerr << Mint(529680742) * 49 << endl;
    for (ll k = 1; k <= n; k++) {
        ll x = a[k - 1];
        Mint les = s.order_of_key(mp(x, INF));  // elements lesser than this current number
        Mint val = x;
        Mint numAdd1 = (val * les * Mint(2)) + val;
        Mint moreSum = st.query(x + 1, mx);
        Mint moreCount = Mint(s.size()) - les;
        Mint numAdd2 = moreSum * 2;
        Mint num = prevNum + numAdd1 + numAdd2;
        Mint den = Mint(k) * Mint(k);
        Mint ans = num / den;
        prevNum = num;
        st.update(x, x);
        s.insert({x, k});

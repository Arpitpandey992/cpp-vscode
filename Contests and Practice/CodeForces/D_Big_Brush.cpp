...
pair<int, bool> chk(int i, int j) {
    if (i < 0 || i >= n || j < 0 || j >= m) return {0, false};
    int v = a[i][j];
    if (done[i][j] || i + 1 >= n || j + 1 >= m) return {0, false};
    fo(k, 4) {
        if (a[i + dx[k]][j + dy[k]] != -1)
            v = a[i + dx[k]][j + dy[k]];
    }
    if (v == -1) return {v, false};
    fo(k, 4) {
        if (a[i + dx[k]][j + dy[k]] != -1 && a[i + dx[k]][j + dy[k]] != v)
            return {v, false};
    }
    return {v, true};
}
...
void solve() {
    ...
    while (q.size()) {
        auto [i, j, v] = q.front();
        q.pop();
        fo(k, 4) a[i + dx[k]][j + dy[k]] = -1;
        ans.pb({i + 1, j + 1, v});
        fo(k, 9) {
            auto [val, flag] = chk(i + dx2[k], j + dy2[k]);
            if (!flag) continue;
            q.push({i + dx2[k], j + dy2[k], val});
            done[i + dx2[k]][j + dy2[k]] = true;
        }
    }
    fo(i, n) {
        fo(j, m) {

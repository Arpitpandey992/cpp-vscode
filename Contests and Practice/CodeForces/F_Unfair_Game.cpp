...
int main() {
    ...
    for (int one = 0; one <= MX; one++) {
        for (int two = 0; two <= MX; two++) {
            for (int three = 0; three <= MX; three++) {
                int oneBit = one + three, twoBit = two + three;
                int cur = ((oneBit % 2 == 0) && (twoBit % 2 == 0)) ? 1 : 0;
                int mx = cur;
                if (one > 0)
                    mx = max(mx, cur + DP[one - 1][two][three]);
                if (two > 0)
                    mx = max(mx, cur + DP[one][two - 1][three]);
                if (three > 0)
                    mx = max(mx, cur + DP[one][two][three - 1]);
                DP[one][two][three] = mx;
                // cout << one << " " << two << " " << three << " " << mx << endl;
            }
        }
    }

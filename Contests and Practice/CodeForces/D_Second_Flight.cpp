...
void solve() {
    ...
    vector<unordered_map<int, ll>> ct1(n), ct2(n);
    vector<int> in(n, 0);
    fo(i, m) {
        int u, v, w;
        cin >> u >> v >> w;
        if (ct1[u - 1].find(v - 1) == ct1[u - 1].end()) {
            adj[u - 1].pb({v - 1, w});
            adj[v - 1].pb({u - 1, w});
            in[u - 1]++;
            in[v - 1]++;
    ...
    // fo(i, n) {
    //     int siz = adj[i].size();
    //     for (int l = 0; l + 1 < siz; l++) {
    //         for (int r = l + 1; r < siz; r++) {
    //             auto &[x1, w1] = adj[i][l];
    //             auto &[x2, w2] = adj[i][r];
    //             ct2[x1][x2] += min(w1, w2);
    //             ct2[x2][x1] += min(w1, w2);
    ...
    while (q--) {
        ...
        if (done[x] || done[y]) {
            if (done[x])
                cout << ct1[x][y] + ct2[x][y] << " ";
            if (done[y])
                cout << ct1[y][x] + ct2[y][x] << " ";
            continue;
        }
        if (in[x] > in[y]) swap(x, y);
        if (!done[x]) {
            for (auto &[next, w1] : adj[x])
                for (auto &[nextNext, w2] : adj[next])
                    ct2[x][nextNext] += min(w1, w2);
            done[x] = true;
        }
        cout << ct1[x][y] + ct2[x][y] << " ";
    }

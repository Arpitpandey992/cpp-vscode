...
class MST {
   ...
   public:
    MST(vector<int> &a) {
        this->a = a;
        siz = 1 << int(ceil(log2(a.size())));
        st.resize(2 * siz);
        for (int i = siz; i - siz < a.size(); i++) {
            st[i] = {a[i - siz]};
        }
        for (int ii = siz - 1; ii > 0; ii--) {
            vector<int> &a = st[ii * 2], &b = st[ii * 2 + 1];
            vector<int> &c = st[ii];
    ...
    int qry1(int i) {
        return lessEqual(0, i - 1, a[i]);
    }
    int qry2(int i) {
        return more(i + 1, a.size() - 1, a[i]);
    }
};

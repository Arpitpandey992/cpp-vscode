...
void solve() {
    ...
    auto getBikes = [&]() {
        auto canBuy = [&](int k) {
            ...
            ll cur = tot;
            while (j >= 0) {
                if (i < 0 || a[i] + cur < p[j]) return false;
                ll left = max(0ll, p[j] - a[i]);
                cur -= left;
                j--;
                i--;
            }
    ...
    auto getMinCost = [&](int bikes) {
        ll sum = 0;
        for (ll i = 0; i < bikes; i++)
            sum += p[i];
        return max(0ll, sum - tot);
    };

    ll bikes = getBikes();
    cout << bikes << " " << getMinCost(bikes) << endl;
}

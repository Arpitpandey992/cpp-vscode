...
void solve() {
    ...
    vector<double> suff(n + 1);
    suff[n] = 0;
    for (int i = n - 1; i >= 0; i--) {
        suff[i] = max(t[i] + a[i], suff[i + 1]);
    }
    a.push_back(100000000000);
    double pref = -100000000000;
    double ans = 100000000000, ansi = -1;
    for (int i = 0; i < n; i++) {
        pref = max(pref, t[i] - a[i]);
        auto rig = suff[i + 1];
        double p = (rig - pref) / 2;
        p = max(p, a[i]);
        p = min(p, a[i + 1]);
        double cur = max(pref + p, rig - p);
        if (cur < ans) {
            ans = cur;
            ansi = p;
        }
    }
    cout << FIX(ansi, 50) << endl;
}

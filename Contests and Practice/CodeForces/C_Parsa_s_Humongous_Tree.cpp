...
ll dfs(int i, vb& vis, vvi& adj, vll& l, vll& r, vll& a) {
    vis[i] = true;
    ll ans1 = 0;
    for (auto x : adj[i]) {
        if (!vis[x]) {
            if (DP[x][0] == -1) {
                a[x] = l[x];
                DP[x][0] = dfs(x, vis, adj, l, r, a);
            }
            if (DP[x][1] == -1) {
                a[x] = r[x];
                DP[x][1] = dfs(x, vis, adj, l, r, a);
            }
            /* if (abs(a[i] - l[x]) >= abs(a[i] - r[x])) {
                ans1 += abs(a[i] - l[x]);
                a[x] = l[x];
            } else {
                ans1 += abs(a[i] - r[x]);
            ...
            ans1 += max(abs(a[i] - l[x]) + DP[x][0], abs(a[i] - r[x]) + DP[x][1]);
        }
    }
    vis[i] = false;
    return ans1;
...
int main() {
    ...
    test {
        ...
        vb vis(n, false);
        fo(i, n) fo(j, 2) DP[i][j] = -1;
        a[0] = l[0];
        ll ans1 = dfs(0, vis, adj, l, r, a);
        a.clear();
        a.resize(n, -1);
        vis.clear();
        vis.resize(n, false);
        a[0] = r[0];
        ll ans2 = dfs(0, vis, adj, l, r, a);

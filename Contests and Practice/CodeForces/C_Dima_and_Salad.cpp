...
ll dp(int i, int del) {
    ...
    if (DP.find(mp(i, del)) != DP.end())
        return DP[mp(i, del)];
    ll ans = dp(i + 1, del);
    if (del + a[i] - b[i] >= -10000)
        ans = max(ans, a[i] + dp(i + 1, del + (a[i] - b[i])));
    return DP[mp(i, del)] = ans;
}
...
void solve() {
    ...
    // vector<vector<unordered_set<ll>>> DP1(n + 1, vector<unordered_set<ll>>(mx + 1));

    // // ll DP[n + 1][mx + 1];

    // for (int sum = mx; sum > 0; sum--) {
    //     for (auto &bi : DP1[0][sum]) {
    //         if (bi == sum) {
    //             cout << sum << endl;
    //             return;
    //         }
...
// DP[i][sum] = number of ways to choose when fruits are chosen from i to n-1,
//              and sum of sweet = sum of calo where sum of sweet = sum

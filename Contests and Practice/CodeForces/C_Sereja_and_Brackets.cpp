...
tiii st[4 * N];
//Max complete seq, open brackets not in ans, closed brackets not in ans
tiii build(int l, int r, int node) {
    if (l > r) return {0, 0, 0};
    if (l == r) {
        bool flag = s[l] == '(';
        return st[node] = {0, (flag ? 1 : 0), (flag ? 0 : 1)};
    }
    int mid = midpoint(l, r);
    auto [a1, b1, c1] = build(l, mid, node * 2 + 1);
    auto [a2, b2, c2] = build(mid + 1, r, node * 2 + 2);
    int t = min(b1, c2);
    return st[node] = {a1 + a2 + t * 2, b1 + b2 - t, c1 + c2 - t};
}

tiii query(int ql, int qr, int l, int r, int node) {
    if (qr < l || ql > r || l > r) return {0, 0, 0};
    if (l >= ql && r <= qr) return st[node];
    int mid = midpoint(l, r);
    auto [a1, b1, c1] = query(ql, qr, l, mid, node * 2 + 1);
    auto [a2, b2, c2] = query(ql, qr, mid + 1, r, node * 2 + 2);
    int t = min(b1, c2);
    return {a1 + a2 + 2 * t, b1 + b2 - t, c1 + c2 - t};
}
...
void solve() {
    ...
    while (q--) {
        int l, r;
        cin >> l >> r;
        cout << get<0>(query(l - 1, r - 1, 0, n - 1, 0)) << endl;
    }
}

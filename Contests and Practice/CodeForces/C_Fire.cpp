...
void solve() {
    ...
    // i, range -> 100, 2000
    ll mxRange = 2001;
    vector<vector<ll>> DP(n, vector<ll>(mxRange + 1, -1));
    vector<vector<bool>> take(n, vector<bool>(mxRange + 1));

    auto dp = [&](ll i, ll range, auto &dp) -> int {
        if (i == n) return 0;
        if (DP[i][range] != -1) return DP[i][range];
        ll j = a[i];
        ll ans1 = 0;
        // save this item
        if (t[j] < min(range, d[j])) {
            ans1 = p[j] + dp(i + 1, min(range, d[j]) - t[j], dp);
        }
        // ignore this item
        ll ans2 = dp(i + 1, range, dp);
        if (ans1 > ans2) {
            take[i][range] = true;
            DP[i][range] = ans1;
        } else {
    ...
    cout << dp(0, mxRange, dp) << endl;
    vector<int> ans;
    ll range = mxRange;
    for (int i = 0; i < n; i++) {
        if (take[i][range]) {
            ans.pb(a[i]);
            range = min(range, d[a[i]]) - t[a[i]];
        }
    }
    cout << ans.size() << endl;
    reverse(all(ans));

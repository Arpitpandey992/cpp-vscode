...
int mnxor(tri *t, int x) {
    int ans = 0;
    for (int i = 31; i >= 0; i--) {
        int bit = (x >> i) & 1;
        if (t->next[bit] && t->next[bit]->ct > 0)
            t = t->next[bit];
        else {
            ans ^= (1 << i);
            t = t->next[!bit];
...
int best(tri *root, int x, int i, bool flag) {
    if (!root) return 0;
    if (flag) return root->mx;
    if (i < 0) return 0;
    int bit = (x >> i) & 1;
    int bitk = (k >> i) & 1;
    if (bitk)
        return best(root->next[!bit], x, i - 1, flag);
    int op1 = best(root->next[!bit], x, i - 1, true);
    int op2 = best(root->next[bit], x, i - 1, flag);
    return max(op1, op2);
}

void ins(tri *root, int x, int val, int i) {
    root->ct++;
    root->mx = max(root->mx, val);
    if (i < 0) return;
    int bit = (x >> i) & 1;
    if (!root->next[bit])
        root->next[bit] = new tri();
    ins(root->next[bit], x, val, i - 1);
}
...
void solve() {
    ...
    sort(all(a));
    tri *root = new tri();
    ins(root, a[0].ff, 1, 31);
    vi DP(n, 1);
    for (int i = 1; i < n; i++) {
        int bst = best(root, a[i].ff, 31, false);
        DP[i] = 1 + bst;
        ins(root, a[i].ff, DP[i], 31);
    }
    auto it = max_element(all(DP)) - DP.begin();
    if (n > 100) {
        cout << DP[it] << endl;
        return;
    }
    ...
    while (it >= 0) {
        ...
        if (it == 0) break;
        int i = it - 1;
        while (i >= 0 && ((a[i].ff ^ a[it].ff) < k) || DP[i] + 1 != DP[it]) {
            i--;
        }
        it = i;
    }

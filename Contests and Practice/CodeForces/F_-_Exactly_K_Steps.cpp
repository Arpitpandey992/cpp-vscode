...
void dfs1(int i, int p, vvi &par, int cur = 0) {
    par[i][0] = p;
    dep[i] = cur;
    for (auto &x : adj[i])
        if (x != p) dfs1(x, i, par, cur + 1);
}

ll dfs2(int i, int p, vvi &down) {
    int ans = 1;
    for (auto &x : adj[i]) {
        if (x == p) continue;
        int val = dfs2(x, i, down);
        if (val + 1 >= ans) {
            ans = val + 1;
            down[i][0] = x;
        }
...
int binary_lift(int i, int k, vvi &par) {
...
void solve() {
    cin >> n;
    adj.resize(n);
    vector<vector<int>> par, down;
    par.resize(n, vector<int>(LOG, -1));
    down.resize(n, vector<int>(LOG, -1));
    dep.resize(n, 0);
    subDep.resize(n, 0);
    fo(i, n - 1) {
        int u, v;
    ...
    for (int lev = 1; lev < LOG; lev++) {
        fo(i, n) {
            if (par[i][lev - 1] != -1) par[i][lev] = par[par[i][lev - 1]][lev - 1];
            if (down[i][lev - 1] != -1) down[i][lev] = down[down[i][lev - 1]][lev - 1];
        }
    }
    int q;
    cin >> q;
    while (q--) {
        ...
        if (dep[u] >= k) {
            cout << binary_lift(u, k, par) + 1 << endl;
            continue;
        }
        int subPar = binary_lift(u, dep[u] - 1, par);
        // if (dep[u] == k - 1) {
        //     bool flag = false;
        //     for (auto &x : adj[0]) {
        //         if (x != subPar) {
        ...
        //     continue;
        // }
        if (aa.size() >= 1 && aa[0].ss != subPar) {
            int val = binary_lift(aa[0].ss, k - dep[u] - 1, down);
            if (val != -1)
                cout << val + 1 << endl;
            else
                cout << -1 << endl;
        } else if (aa.size() >= 2) {
            int val = binary_lift(aa[1].ss, k - dep[u] - 1, down);
            if (val != -1)
                cout << val + 1 << endl;
            else
                cout << -1 << endl;

...
Mint getVal(int a, int b, int K) {
    // 1,2,3,......,a,..........,b,.....................n-1,n;
    Mint DP[b + 1][K + 1];
    vector<Mint> pref(b + 1);
    pref[0] = 0;
    for (int k = 0; k <= K; k++) {
        auto ppref = pref;
        for (int i = 1; i < b; i++) {
            if (k == 0) {
                DP[i][k] = 1;
                ppref[i] = DP[i][k] + ppref[i - 1];
                continue;
            }
            // DP[i] -> DP[i-len] to DP[i+len];
            int len = b - i;
            int l = max(1, i - len + 1), r = min(b - 1, i + len - 1);
            DP[i][k] = pref[r] - pref[l - 1] - DP[i][k - 1];
            
            ppref[i] = ppref[i - 1] + DP[i][k];
        }
        pref = ppref;
    }
    return DP[a][K];

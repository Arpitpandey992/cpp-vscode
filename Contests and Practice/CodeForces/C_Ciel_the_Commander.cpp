...
int dfs1(int i, int par, vi adj[], vpii &siz) {
    int &mx = siz[i].ff, &smx = siz[i].ss;
    for (auto x : adj[i]) {
        if (x == par) continue;
        int t = dfs1(x, i, adj, siz);
        if (t + 1 > mx) {
            smx = mx;
            mx = t + 1;
        } else if (t + 1 > smx)
...
void dfs2(int i, int par, vi adj[], vpii &siz, int prev, int &ans, int &cur) {
    auto &[mx, smx] = siz[i];
    if (max(prev + 1, mx) < cur) {
        cur = max(prev + 1, mx);
        ans = i;
    }
    for (auto x : adj[i]) {
        ...
        int nprev = prev + 1;
        if (siz[x].ff + 1 == mx)
            nprev = max(nprev, 1 + smx);
        else
            nprev = max(nprev, 1 + mx);
        dfs2(x, i, adj, siz, nprev, ans, cur);
    }
}

void dfs3(int i, int par, vi adj[], vector<char> &ans, char c) {
    ans[i] = c;
    for (auto x : adj[i]) {
        if (x != par)
            dfs3(x, i, adj, ans, c + 1);
    }
}

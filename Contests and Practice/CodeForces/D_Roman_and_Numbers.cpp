...
ll dp(ll mask, ll mod, string &s, ll left, vvll &DP) {
    if (left == 0) {
        return mod == 0 ? 1 : 0;
    }
    ...
    for (ll i = 0; i < s.size();) {
        ...
        if (s[i] == '0' && left == s.size()) {
            i++;
            continue;
        }
        ll val = s[i] - '0';
        ans += dp(mask ^ (1 << i), (mod * 10 + val) % m, s, left - 1, DP);
        ll j = i;
...
void solve() {
    ...
    Sort(s);
    // for a given mask -> denoting the used indices -> we can fill the rest easily, while maintaining modulo
    ll left = s.size();
    ll maxMask = 1 << left;
    vector<vector<ll>> DP(maxMask, vector<ll>(m + 1, -1));
    cout << dp(0, 0, s, left, DP) << endl;
}

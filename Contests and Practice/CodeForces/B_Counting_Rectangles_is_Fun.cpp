...
int dp(int i, int j, int k, int l) {
    ...
    int ans = 0, cur = INF;
    for (int p = i; p <= k; p++) {
        cur = min(cur, min(suff[p][j], l - j + 1));
        ans += cur;
    }
    return DP[i][j][k][l] = ans + dp(i + 1, j, k, l) + dp(i, j + 1, k, l) - dp(i + 1, j + 1, k, l);
}
...
void solve() {
    ...
    //                     continue;
    //                 }
    //                 bool op1 = (k > i ? yes[i][j][k - 1][l] : true);
    //                 bool op2 = (l > j ? yes[i][j][k][l - 1] : true);
    //                 yes[i][j][k][l] = op1 & op2;
    //             }
    //         }
    //     }

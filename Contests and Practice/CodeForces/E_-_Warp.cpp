...
void solve() {
    ...
    vector<vector<vector<int>>> DP(n + 1, vvi(n + 1, vi(n + 1)));
    for (int m1 = n; m1 >= 0; m1--) {
        for (int m2 = n - m1; m2 >= 0; m2--) {
            for (int m3 = n - m1 - m2; m3 >= 0; m3--) {
                int N = m1 + m2 + m3;
                int x = m1 * a + m2 * c + m3 * e, y = m1 * b + m2 * d + m3 * f;
                if (cant.find(make_pair(x, y)) != cant.end()) {
                    DP[m1][m2][m3] = 0;
                    continue;
                }
                if (N == n) {
                ...
                int ans = 0;
                ans = (ans + DP[m1 + 1][m2][m3]) % M;
                ans = (ans + DP[m1][m2 + 1][m3]) % M;
                ans = (ans + DP[m1][m2][m3 + 1]) % M;
                DP[m1][m2][m3] = ans;
            }
        }
    }

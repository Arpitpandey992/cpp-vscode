...
pair<int, string> DP[101][101][13];
string a[101];
int n, m, k;
int dx[] = {-1, -1};
int dy[] = {-1, 1};
char mov[] = {'L', 'R'};
pair<int, string> dp(int i, int j, int perc) {
    if (j < 0 || j >= m) return mp(-1, "");
    if (i < 0) return ((perc == 0) ? mp(0, "") : mp(-1, ""));
    if (DP[i][j][perc].ff != -2)
        return DP[i][j][perc];
    int pea = a[i][j] - '0';
    int cur = (perc + pea) % k;
    ...
    fo(l, 2) {
        ...
        auto [cnt, ss] = dp(x, y, cur);
        if (cnt != -1 && cnt + pea > ans) {
            ans = cnt + pea;
            sss = mov[l] + ss;
        }
    }
    return DP[i][j][perc] = mp(ans, sss);
}
void solve() {
    cin >> n >> m >> k;
    k++;

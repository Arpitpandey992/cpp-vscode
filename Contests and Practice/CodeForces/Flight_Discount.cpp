...
void solve() {
    ...
    vvll dis(n, vll(2, INFLL/10));
    priority_queue<tlll, vector<tlll>, greater<tlll>> q;
    q.push({0, 0, 0});
    // distance from 0, has special node been used, which node
    dis[0][0] = 0;
    while (q.size()) {
        auto [distance, coupon, node] = q.top();
        q.pop();
        if (distance != dis[node][coupon]) continue;
        for (auto &[x, w] : adj[node]) {
            if (dis[x][coupon] > distance + w) {
                dis[x][coupon] = distance + w;
                q.push({dis[x][coupon], coupon, x});
            }
            // using coupon
            if (coupon == 0) {
                if (dis[x][1] > distance + floor(w, 2)) {
                    dis[x][1] = distance + floor(w, 2);
                    q.push({dis[x][1], 1, x});
                }
            }
        }

...
pii dfs1(int i, int par, vi adj[], vb &vis, vi &a, int &ms) {
    vis[i] = true;
    a.pb(i);
    int mx = 0, smx = 0;
    for (auto x : adj[i]) {
        if (x == par) continue;
        auto [ans, sans] = dfs1(x, i, adj, vis, a, ms);
        if (sans > mx) {
            smx = mx;
            mx = sans;
        } else if (sans > smx)
    ...
    ms = max(ms, mx + smx + 1);
    return {mx + smx + 1, mx + 1};
}

int fin(int x) {
    return par[x] = (x == par[x] ? x : fin(par[x]));
}

void uni(int x, int y) {
    int px = fin(x), py = fin(y);
    if (px == py) return;
    int &sx = siz[px], &sy = siz[py];
    if (sx > sy) {
        par[py] = px;
        sx = max(sx, (int)(ceil(sx + 1, 2) + ceil(sy + 1, 2)));
    } else {
        par[px] = py;
        sy = max(sy, (int)(ceil(sx + 1, 2) + ceil(sy + 1, 2)));
    }
}

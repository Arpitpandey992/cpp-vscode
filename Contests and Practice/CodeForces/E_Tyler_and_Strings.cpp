...
vector<int> st(4 * mxn + 1, 0);
vector<int> ct(mxn + 1, 0);
int qry(int l, int r, int node, int sl, int sr) {
    if (l > sr || r < sl) return 0;
    if (sl >= l && sr <= r)
        return st[node];
    int mid = midpoint(sl, sr);
    return qry(l, r, node * 2 + 1, sl, mid) + qry(l, r, node * 2 + 2, mid + 1, sr);
}

void upd(int x, int d, int node, int sl, int sr) {
    if (x < sl || x > sr) return;

    st[node] += d;
    if (sl == sr) return;
    int mid = midpoint(sl, sr);
    upd(x, d, node * 2 + 1, sl, mid);
    upd(x, d, node * 2 + 2, mid + 1, sr);
}

Mint fact[mxn];
Mint dp(int i, ll rem) {
    if (i == m) return 0;
    if (rem == 0) return 1;
    ll sm = qry(0, b[i] - 1, 0, 0, mxn) % M;
    Mint ans = sm * fact[rem - 1];
    if (ct[b[i]]) {
        ct[b[i]]--;
        if (ct[b[i]] == 0)
            upd(b[i], -1, 0, 0, mxn);

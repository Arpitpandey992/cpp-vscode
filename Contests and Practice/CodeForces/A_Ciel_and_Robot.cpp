...
#pragma endregion
//-------------------------------------------------------------------------------//
map<char, pii> m = {{'D', {0, -1}}, {'U', {0, 1}}, {'L', {-1, 0}}, {'R', {1, 0}}};
ll a, b;
string s;
vpii v;
...
bool solve() {
    ...
    for (auto [x, y] : v) {
        if (dx == 0 && dy == 0) {
            ...
            if (a != x || !same(b - y, dy) || ((b - y) % dy != 0)) 
            continue;
            return true;
        } else if (dy == 0) {
            if (b != y || !same(a - x, dx) || (a - x) % dx != 0) 
            continue;
            return true;
        } else {
            if (!same(b - y, dy) || ((b - y) % dy != 0) || !same(a - x, dx) || (a - x) % dx != 0)
                continue;
            if ((a - x) * dy == (b - y) * dx) {
                return true;
            }

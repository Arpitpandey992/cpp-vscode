...
ll dp(ll i, ll sum, ll any) {
    ...
    if (any) {
        for (ll j = 0; j < 10; j++)
            ans += dp(i + 1, sum - j, any);
    } else {
        ll dig = s[i] - '0';
        for (ll j = 0; j < dig; j++)
            ans += dp(i + 1, sum - j, true);
        ans += dp(i + 1, sum - dig, any);
    }
    return DP[i][sum][any] = ans;
}
...
vector<ll> num(ll k) {
    ...
    vector<ll> ans(200, 0);
    if (k <= 0) return ans;
    for (ll sum = 1; sum < 200; sum++) {
        ans[sum] += dp(0, sum, false);
    }
    return ans;
}

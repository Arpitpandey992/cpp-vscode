...
pair<int, int> dfs(int i, int par) {
    ...
    for (auto &j : adj[i]) {
        ...
        auto p = dfs(j, i);
        if (p.ff != -1) {
            if (p.ff + 1 > m1 || p.ff + 1 == m1 && p.ss < i1) {
                m2 = m1;
                i2 = i1;
                m1 = p.ff + 1;
                i1 = p.ss;
            } else if (p.ff + 1 > m2 || p.ff + 1 == m2 && p.ss < i2) {
                m2 = p.ff + 1;
                i2 = p.ss;
            }
        }
...
int DP[123499][2];
int dp(int i, int typ, int par = -1) {
    bool flag = sp[i];
    if (DP[i][typ] != -1) return DP[i][typ];
    if (typ == 0) {  // end anywhere -> choose x-1 to end on start node, then 1 to end anywhere
        // hence cost = 2*(x-1) + sum(dp(j, 1))[those x-1 nodes] + 1 + dp(k, 0);
        vector<pair<int, int>> v;
        int sum_1 = 0;
        for (auto &j : adj[i]) {
            if (j == par) continue;
            v.push_back({2 + dp(j, 1, i), 1 + dp(j, 0, i)});
            if (v.back().first == 1) {  // function returned -1
                v.pop_back();
            } else {
                sum_1 += v.back().first;
                flag = true;
        ...
        int ans = INT_MAX / 2;
        for (auto &[lef, rig] : v) {
            ans = min(ans, sum_1 - lef + rig);
        }
        return DP[i][typ] = ans;
    }
    // typ==1 -> start and end on the same point
    // traverse through all nodes -> 2(x) + sum(dp(j,1))
    vector<int> v;
    int ans = 0;
    for (auto &j : adj[i]) {
        if (j == par) continue;

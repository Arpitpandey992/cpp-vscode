...
int dfs(int i, vector<vector<int>> &adj, int par, int cur, vector<int> &a, vector<int> &siz, vector<int> &h) {
    siz[i] = 1;
    h[i] = cur;
    a.pb(i);
    for (auto &x : adj[i]) {
        if (x == par) continue;
        siz[i] += dfs(x, adj, i, cur + 1, a, siz, h);
    }
    return siz[i];
}
...
void solve() {
    ...
    vector<int> euler, siz(n, 0), h(n, 0);
    dfs(0, adj, -1, 0, euler, siz, h);
    map<int, int> pos;
    fo(i, n) pos[euler[i]] = i;
    vll b1(n, 0), b2(n, 0);
    for (auto &i : euler) {
        if (h[i] % 2 == 0)
            b1[pos[i]] = a[i];
    ...
    vll diff1(n), diff2(n);
    fo(i, n) {
        diff1[i] = b1[i] - (i > 0 ? b1[i - 1] : 0);
        diff2[i] = b2[i] - (i > 0 ? b2[i - 1] : 0);
    }
    segTree ST1(diff1);
    segTree ST2(diff2);
    while (m--) {
        ...
        if (typ == 2) {
            ...
            int ind = pos[i];
            if (h[i] & 1)
                cout << ST2.query(0, pos[i]) << endl;
            else
                cout << ST1.query(0, pos[i]) << endl;
        } else {
            int i, x;
            cin >> i >> x;
            i--;

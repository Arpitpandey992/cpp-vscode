...
//-------------------------------------------------------------------------------//
int DP[101][10001];
int dp(int i, ll sum, vll &a, vll &b) {
    if (sum < 0) return 0;
    if (i == a.size()) {
        if (sum == 0) return 1;
        return 0;
    ...
    if (dp(i + 1, sum - a[i], a, b) || dp(i + 1, sum - b[i], a, b))
        return DP[i][sum] = 1;
    return DP[i][sum] = 0;
}
...
void solve() {
    ...
    fo(i, n) cin >> a[i];
    fo(i, n) cin >> b[i];
    ll mn = 0, mx = 0, tsum = 0, idsq = 0;
    if (n == 1) {
        cout << 0 << endl;
        return;
    }
    fo(i, n) {
        idsq += (a[i] * a[i] + b[i] * b[i]);
        tsum += a[i] + b[i];
        mn += min(a[i], b[i]);
        mx += max(a[i], b[i]);
    }
    ll ans = INF;
    for (ll sum = mn; sum <= mx; sum++) {
        if (dp(0, sum, a, b)) {
            ans = min(ans, sum * sum + (tsum - sum) * (tsum - sum) + (n - 2) * idsq);
        }
    }
    cout << ans << endl;
}

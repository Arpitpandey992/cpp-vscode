...
pair<double, double> one(double a, double b) {
    return {div(a * s, a + b), div(b * s, a + b)};
}

td fun(double a, double b, double c, double s) {
    if (b == 0 && c == 0) return {s, b, c};
    if (a == 0 && c == 0) return {a, s, c};
    if (a == 0 && b == 0) return {a, b, s};
    if (a == 0) return {a, one(b, c).ff, one(b, c).ss};
    if (b == 0) return {one(a, c).ff, b, one(a, c).ss};
    if (c == 0) return {one(a, b).ff, one(a, b).ss, c};
    double y = div((b * c * s), ((a + c) * (b + c) - a * b));
    double x = div((a * s - a * y), (a + c));
    double z = s - x - y;
    return {x, y, z};
}

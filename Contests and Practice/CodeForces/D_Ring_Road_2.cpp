...
int sid(int u, int v, int p) {
    if (p == u || p == v) return -1;
    if (p > u && p > v || p < u && p < v) return 0;
    return 1;
}
int insect(int u1, int v1, int u2, int v2) {
    auto p = mp(sid(u1, v1, u2), sid(u1, v1, v2));
    if (p.ff == -1 || p.ss == -1 || p.ff == p.ss) return 0;
    return 1;
}
bool build(int i, vi adj[], vb &vis, vi &ans, int cur) {
    if (vis[i]) {
        if (ans[i] == cur) return true;
        return false;
    }
    vis[i] = true;
    ans[i] = cur;
    for (auto x : adj[i]) {
        if (!build(x, adj, vis, ans, !cur))
            return false;
    }
    return true;
}
...
void solve() {
    ...
    fo(i, m) {
        fo(j, m) {
            if (i == j) continue;
            if (insect(ed[i].ff, ed[i].ss, ed[j].ff, ed[j].ss))
                adj[i].pb(j);
        }
    }
    vector<int> ans(m);

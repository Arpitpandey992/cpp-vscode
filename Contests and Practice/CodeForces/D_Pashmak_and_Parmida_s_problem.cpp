...
#define N 1000001
int st[4 * N];
int update(int l, int r, int node, int val) {
    if (l > val || r < val) return st[node];
    if (l == r) {
        st[node]++;
        return st[node];
    }
    int mid = midpoint(l, r);
    return st[node] = update(l, mid, node * 2 + 1, val) +
                      update(mid + 1, r, node * 2 + 2, val);
}
...
int query(int l, int r, int node, int val) {
    ...
    if (r < val)
        return st[node];
    int mid = midpoint(l, r);
    return query(l, mid, node * 2 + 1, val) +
           query(mid + 1, r, node * 2 + 2, val);
}

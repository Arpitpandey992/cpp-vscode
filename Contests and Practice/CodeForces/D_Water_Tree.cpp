...
class segTree {
    ...
    void pointUpdate(int sl, int sr, int node, int i) {
        if (lazy[node] != 0) {
            st[node] = sr - sl + 1;
            if (sl != sr) {
                lazy[node * 2 + 1] = 1;
        ...
        if (sl > i || sr < i)  // completely outside or inside -> done
            return;
        if (sl == sr) {  // completely inside -> lazily set this bit
            st[node] = st[node] - 1;
            return;
        }
        int mid = (sl + sr) / 2;
        pointUpdate(sl, mid, node * 2 + 1, i);
        pointUpdate(mid + 1, sr, node * 2 + 2, i);
        st[node] = st[node * 2 + 1] + st[node * 2 + 2];
    }
    void upd(int sl, int sr, int node, int l, int r) {
        // lazy values -> 0 = set all subtree to 0, 1-> set to 1, 2->flip all bits
        if (lazy[node] != 0) {
            st[node] = sr - sl + 1;
            if (sl != sr) {
                lazy[node * 2 + 1] = 1;
        ...
        if (sl >= l && sr <= r) {  // completely inside -> lazily set this bit
            st[node] = sr - sl + 1;
            if (sl != sr) {
                lazy[node * 2 + 1] = 1;
                lazy[node * 2 + 2] = 1;
        ...
        if (sl > r || sr < l)  // completely outside or inside -> done
            return;
        int mid = (sl + sr) / 2;
        upd(sl, mid, node * 2 + 1, l, r);
        upd(mid + 1, sr, node * 2 + 2, l, r);
        st[node] = st[node * 2 + 1] + st[node * 2 + 2];
    }
    int qry(int sl, int sr, int node, int l, int r) {
        if (lazy[node] != 0) {
            st[node] = sr - sl + 1;
            if (sl != sr) {
                lazy[node * 2 + 1] = 1;
        ...
        if (sl >= l && sr <= r) {  // completely inside
            return st[node];
        }
        if (sl > r || sr < l)  // completely outside -> done
            return 0;

        int mid = (sl + sr) / 2;
        return qry(sl, mid, node * 2 + 1, l, r) + qry(mid + 1, sr, node * 2 + 2, l, r);
    }
   ...
   public:
    ...
    void rangeSet(int l, int r) {
        upd(0, n - 1, 0, l, r);
    }
    void unset(int i) {
        pointUpdate(0, n - 1, 0, i);
    }
...
void dfs(int i, int par, int &curTime, vpii &timer, vvi &adj) {
    timer[i].ff = curTime;
    for (auto &x : adj[i]) {
        if (x == par) continue;
        dfs(x, i, ++curTime, timer, adj);
    }
    timer[i].ss = curTime;
}
...
void solve() {
    ...
    while (q--) {
        ...
        v--;
        if (c == 1) {
            st.rangeSet(timer[v].ff, timer[v].ss);
        } else if (c == 2) {
            st.unset(timer[v].ff);
        } else if (c == 3) {
            int setBits = st.query(timer[v].ff, timer[v].ss);
            if (setBits == timer[v].ss - timer[v].ff + 1)
                cout << 1 << endl;
            else
                cout << 0 << endl;
        } else {
            fo(i, n) {
                cout << i + 1 << " -> ";
                int setBits = st.query(timer[i].ff, timer[i].ss);
                if (setBits == timer[i].ss - timer[i].ff + 1)
                    cout << 1 << endl;
                else
                    cout << 0 << endl;
            }

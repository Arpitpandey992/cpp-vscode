...
void solve() {
    ...
    fo(i, n) cin >> x[i];
    vll tp[n];
    map<pair<ll, ll>, vector<tuple<ll, ll, ll>>> adj;
    fo(i, k) {
        ll a, b, c, d, h;
        cin >> a >> b >> c >> d >> h;
        tp[a - 1].pb(b - 1);
        tp[c - 1].pb(d - 1);
        adj[mp(a - 1, b - 1)].pb({c - 1, d - 1, -h});
    }
    tp[n - 1].pb(m - 1);
    tp[0].pb(0);
    for (ll i = 0; i < n; i++) {
        Sort(tp[i]);
        for (ll j = 0; j + 1 < tp[i].size(); j++) {
            ll c1 = tp[i][j], c2 = tp[i][j + 1];
            adj[mp(i, c1)].pb({i, c2, x[i] * (c2 - c1)});
            adj[mp(i, c2)].pb({i, c1, x[i] * (c2 - c1)});
        }
    }
#define tiii tuple<ll, ll, ll>
    ...
    while (q.size()) {
        auto [dis, x, y] = q.top();
        q.pop();
        if (dis > d[mp(x, y)]) continue;
        // if (mp(x, y) == mp(n - 1, m - 1)) {
        //     cout << dis << endl;
        //     return;
        // }
        for (auto [nx, ny, w] : adj[mp(x, y)]) {
            if (nx == x) {
                if (!CHK(d, mp(nx, ny)) || d[mp(nx, ny)] > dis + w) {
                    d[mp(nx, ny)] = dis + w;
                    q.push({dis + w, nx, ny});
                }
            }

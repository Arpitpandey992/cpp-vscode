...
const int N = 1e6 + 1;
vi st[4 * N];
void build(int l, int r, int node, vi &aa) {
    if (l == r) {
        st[node] = {aa[l]};
        return;
    }
    int mid = (l + r) / 2;
    build(l, mid, node * 2 + 1, aa);
    build(mid + 1, r, node * 2 + 2, aa);
    vi &a = st[node * 2 + 1];
    vi &b = st[node * 2 + 2];
    vi &c = st[node];
    int i = 0, j = 0;
...
int query1(int ql, int qr, int l, int r, int node, int val) {
    if (ql > r || qr < l) return 0;
    if (l >= ql && r <= qr)
        return lb(st[node], val) - st[node].begin();
    int mid = (l + r) / 2;
    return query1(ql, qr, l, mid, node * 2 + 1, val) + query1(ql, qr, mid + 1, r, node * 2 + 2, val);
}
int query2(int ql, int qr, int l, int r, int node, int val) {
    if (ql > r || qr < l) return 0;
    if (l >= ql && r <= qr)
        return (r - l + 1) - (ub(st[node], val) - st[node].begin());
    int mid = (l + r) / 2;
    return query2(ql, qr, l, mid, node * 2 + 1, val) + query2(ql, qr, mid + 1, r, node * 2 + 2, val);
}
...
void solve() {
    ...
    ll ans = 0;
    for (int i = 0; i < n; i++) {
        lef[i] = (i > 0 ? query2(0, i - 1, 0, n - 1, 0, a[i]) : 0);
        rig[i] = (i < n - 1 ? query1(i + 1, n - 1, 0, n - 1, 0, a[i]) : 0);
        ans += lef[i] * rig[i];
    }
    cout << ans << endl;
}

...
bool can(int god, vector<int> out) {
    int k = out[god];
    vector<bool> done(n, false);
    done[god] = true;
    priority_queue<pair<int, int>> q;
    fo(i, n) if (!done[i]) q.push({out[i], i});
    while (q.size()) {
        auto [o, t] = q.top();
        q.pop();
        if (o != out[t]) continue;
        ...
        fo(i, n) {
            if (!done[i] && i != t && !adj[t][i] && !adj[i][t])
                v.pb({out[i], i});
        }
        Sort(v);
        int need = v.size() - (k - 1 - o);
        fo(j, need) {
            out[v[j].ss]++;
            q.push({out[v[j].ss], v[j].ss});
        }
        done[t] = true;
    }
    return true;

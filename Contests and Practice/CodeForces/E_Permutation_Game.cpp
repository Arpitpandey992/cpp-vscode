...
int can(int one, int two, int none) {
    if (one + none == 0) return 1;
    if (two + none == 0) return -1;
    if (one == 0 && two == 0) return 0;
    if (none > 0) {
        if (two == 0) {
            return can(one, two, none - 1);  // opp does nothing
        }
        return can(one, two - 1, none - 1);
    }
    if (two > 0)
        return can(one - 1, two - 1, none);
    return can(one - 1, two, none);
}
...
void solve() {
    ...
    if (can(one, two, none) == 1) {
        cout << "First\n";
        return;
    } else if (can(one, two, none) == 0) {
        cout << "Tie\n";
        return;
    }
    if (one > 0) {

...
void dfs(vi &dep, int i, int cur, int par, vector<int> adj[]) {
    dep[i] = cur;
    for (auto &x : adj[i]) {
        if (x == par) continue;
        dfs(dep, x, cur + 1, i, adj);
    }
}
int dfs2(vi &h, int i, int par, vector<int> adj[]) {
    h[i] = 1;
    for (auto &x : adj[i]) {
        if (x == par) continue;
        h[i] += dfs2(h, x, i, adj);
...
void dfs3(int i, int par, vector<int> &a, vi &pos, vector<int> adj[]) {
    a.pb(0);
    pos[i] = a.size() - 1;
...
void solve() {
    ...
    vector<int> dep(n), h(n);  // depth of node, number of nodes after ith node in its family
    dfs(dep, 0, 0, -1, adj);
    dfs2(h, 0, -1, adj);
    vector<int> a, b(n + 1, 0), pos(n);
    dfs3(0, -1, a, pos, adj);
    segTree s1(a), s2(b);
    while (q--) {
        int typ;
        ...
        if (typ == 0) {
            ...
            if (d < dep[v]) {
                ...
                s2.update(left + 1, -x);
                s2.update(0, x);
                int point = p + left + 1 - dep[v];
                if (point - p >= h[v])
                    continue;
                s1.update(point, x);
            }
            if (p + min(d+1, h[v]) < n)
                s1.update(p + min(d+1, h[v]), -x);
        } else {
            int i;
            cin >> i;
            i--;
            int p = pos[i];
            cout << s1.query(0, p) + s2.query(0, dep[i]) << endl;
        }
    }
}

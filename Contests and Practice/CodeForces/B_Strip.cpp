...
class SegTree {
    ...
    bool maximum;
    int cmp(int x, int y) {
        if (maximum) return max(x, y);
        return min(x, y);
    }
    int def() {
        return maximum ? (-INF) / 2 : INF / 2;
    }

   public:
    SegTree(vector<int> &a, bool mx) {
        maximum = mx;
        int n = a.size();
    ...
    int query(int l, int r) {
        ...
        int ans = def();
        while (l <= r) {
            if (l % 2 == 1) ans = cmp(ans, st[l++]);
            if (r % 2 == 0) ans = cmp(ans, st[r--]);
            l /= 2;
            r /= 2;
        }
        return ans;
...
void solve() {
    ...
    fo(i, n) cin >> a[i];
    vi till(n);
    SegTree maxTree(a, true), minTree(a, false);
    fo(i, n) {
        int l = i, r = n;
        while (l < r) {
            int mid = (l + r) / 2;
            int diff = maxTree.query(i, mid) - minTree.query(i, mid);
            if (diff <= s)
                l = mid + 1;
            else
                r = mid;
    ...
    for (int i = n - 1; i >= 0; i--) {
        ...
        DP[i] = 1ll + dpSegTree.query(lef, rig);
        dpSegTree.update(i, DP[i]);
    }
    cout << (DP[0] < (INF/2) ? DP[0] : -1) << endl;
}

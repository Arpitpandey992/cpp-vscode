...
void solve() {
    // DP[dig][mod] = count of numbers having dig digits and number%k == mod
    // DP[dig][mod] = DP[dig-1][0]*(numbers which when added as last digit have modulo = 0)
    //              + DP[dig-1][1]*(numbers -> n%k==0 -> ((n*10)+d)%k==mod) -> ((n%k)*10)%k
    int dig, k, m;
    cin >> dig >> k >> m;
    int DP[dig + 1][k];
    int ans = 0;
    for (int mod = 0; mod < k; mod++) {
        DP[0][mod] = 0;
        DP[1][mod] = 0;
        for (int d = (dig == 1 ? 1 : 0); d < 10; d++) {
            if (d % k == mod)
                DP[1][mod] = (DP[1][mod] + 1) % m;
        }
    }
    if (dig == 1) {
        cout << DP[1][0] << endl;
    ...
    for (int n = 2; n <= dig; n++) {
        for (int mod = 0; mod < k; mod++) {
            ...
            for (int pMod = 0; pMod < k; pMod++) {
                if (mod == pMod) continue;
                int count = 0;
                int val = ((mod - pMod) % k + k) % k;
                for (int d = (n == dig ? 1 : 0); d < 10; d++) {
                    int dModK = (d * modexp(10, dig, k)) % k;
                    if (dModK == val)
                        count++;
                }
                DP[n][mod] = (DP[n][mod] + (DP[n - 1][pMod] * count) % m) % m;
            }
        }
    }
    for (int i = 1; i <= dig; i++) {
        int val = ((DP[i][0] - DP[i - 1][0]) % m + m) % m;
        if (i == dig)
            ans += val;
        else
            ans += (val * modexp(10, dig - i - 1) * 9) % m;
    }
    cout << ans % m << endl;
}

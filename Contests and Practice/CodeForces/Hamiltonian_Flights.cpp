...
ll dp(int mask, int i, vvll &adj, vvll &DP) {
    if (i == n - 1) {
        if (mask + 1 == maxMask) return 1;
        return 0;
    }
    ...
    for (auto &x : adj[i]) {
        ...
        ans = (ans + dp(mask ^ (1 << x), x, adj, DP)) % M;
    }
...
void solve() {
    ...
    vector<vector<ll>> DP(maxMask, vll(n, -1));
    cout << dp(1, 0, adj, DP) << endl;
}

    ...
    for (int i = 0; i < 26; i++) {
        for (int j = 0; j < 26; j++) {
            for (int k = 0; k < 26; k++) {
                ...
                temp += (k + 'a');
                tp.insert(temp);
                tp.insert(temp.substr(0, 1));
                tp.insert(temp.substr(1, 1));
                tp.insert(temp.substr(2, 1));
                tp.insert(temp.substr(0, 2));
                tp.insert(temp.substr(1, 2));
            }
        }
    }
    vector<string> super(all(tp));
    tp.clear();
    sort(all(super), cmp);
    debug(super.size());
    test {

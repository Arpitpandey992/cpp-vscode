const long long M = 1e9 + 7; li for each 1 ≤ i ≤ 
// const int INF = 2147483647 / 2;
const long long INF = 9223372036854775807ll / 2ll;
#pragma region
#include <algorithm>
#include <chrono>
#include <cmath>
...
bool chk(vll &a, int i, int l, int r) {
    ll n1 = (i > l ? a[i - 1] : -INF);
    ll n2 = (i < r ? a[i + 1] : INF);
    return n1 + 1 < n2;
}

bool can(int l, int r, vll &bad, vll &a) {
    auto j = ub(bad, r) - bad.begin() - 1, i = ub(bad, l) - bad.begin();
    if (j < i) return true;
    if (j - i > 0) return false;
    int id = bad[i];
    if (chk(a, id, l, r) || chk(a, id - 1, l, r))
        return true;
    return false;
}
bool ican(vll &a, vll &bad, int siz) {
    for (int i = 0; i + siz - 1 < a.size(); i++) {
        if (can(i, i + siz - 1, bad, a))
            return true;
    }
    return false;
}

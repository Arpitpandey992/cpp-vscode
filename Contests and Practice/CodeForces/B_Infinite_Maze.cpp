...
int dfs(int x, int y) {
    if (a[mod(x, n)][mod(y, m)] == '#')
        return 0;
    if (vis[mod(x, n)][mod(y, m)] != mp(INF, INF)) {
        return 1;
    }
    // cout << x << " " << y << endl;
    // cout << mod(x, n) << " " << mod(y, m) << endl;
    vis[mod(x, n)][mod(y, m)] = mp(x,y);
    fo(k, 4) {
        int nx = x + dx[k];
        int ny = y + dy[k];
        if (vis[mod(nx, n)][mod(ny, m)] != mp(nx, ny) && dfs(nx, ny))
            return 1;
    }
    return 0;
}
...
void solve() {
    ...
    a.resize(n);
    fo(i, n) cin >> a[i];
    fo(i, n) fo(j, m) vis[i][j] = {INF,INF};
    fo(i, n) fo(j, m) if (a[i][j] == 'S') {
        sx = i;
        sy = j;
        break;
    }

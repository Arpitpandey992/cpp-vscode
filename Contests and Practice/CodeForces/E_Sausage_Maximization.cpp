...
class Trie {
    struct node {
        ...
        ll ct;
        node *next[2];
        node() : cur(0), ct(0) { next[0] = next[1] = NULL; }
    };
    node *root;
    void ins(ll x, ll bit, node *cur) {
        cur->ct++;
        if (bit < 0)
            return;
        ll b = (x >> bit) & 1ll;
        if (!cur->next[b])
            cur->next[b] = new node();
        ins(x, bit - 1, cur->next[b]);
    }
    node *del(ll x, ll bit, node *cur) {
        if (bit < 0)
            return NULL;
        cur->ct--;
        if (cur->ct == 0) return NULL;
        ll b = (x >> bit) & 1ll;
        cur->next[b] = del(x, bit - 1ll, cur->next[b]);
        return cur;
    }

    void MXX(ll x, ll bit, node *cur, ll &ans) {
        if (bit < 0) return;
        ll b = (x >> bit) & 1ll;
        if (cur->next[!b]) {
            ans ^= (1ll << bit);
            MXX(x, bit - 1ll, cur->next[!b], ans);
            return;
        }
        MXX(x, bit - 1ll, cur->next[b], ans);
    }
...
void solve() {
    ...
    vector<ll> suff(n);
    for (ll i = n - 1; i >= 0; i--) {
        suff[i] = a[i] ^ (i < n - 1 ? suff[i + 1] : 0);
        T.insert(suff[i]);
    }
    ll cur = 0, ans = 0;
    for (ll i = 0; i < n; i++) {
        ans = max(ans, T.maxXor(cur));
        cur ^= a[i];
        T.delet(suff[i]);
    }
    cout << ans << endl;

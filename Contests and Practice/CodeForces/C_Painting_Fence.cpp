...
ll calc(int l, int r, ll cur, vvi &mn, vi &a) {
    if (l > r) return 0;
    if (l == r) return min(1ll, a[l] - cur);
    int id = mn[l][r];
    ll op1 = a[id] - cur + calc(l, id - 1, a[id], mn, a) +
             calc(id + 1, r, a[id], mn, a);
    ll op2 = r - l + 1;
    return min(op1, op2);
}
...
void solve() {
    ...
    cin >> n;
    vi a(n);
    // fo(i, n) a[i] = rng(0, 100);
    fo(i, n) cin >> a[i];
    vvi mn(n, vi(n));
    fo(i, n) {
        fok(j, i, n) {
            int cont = (j > i ? mn[i][j - 1] : j);
            if (a[cont] < a[j])
                mn[i][j] = cont;
            else
                mn[i][j] = j;

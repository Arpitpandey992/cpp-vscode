...
//if stuck, try building the solution from end to beginning (stamping sequence, bursting balloons, etc)
//check overflow, if a variable is of 10**9 order, use long long only, don't try to look cool by using int and avoiding overflow.
//check for division by zero
//a.size()-1 can underflow!, use int n=a.size(); then use n-1 instead
//be careful if erasing while iterating a set or map
...
bool win(int x, int y, vector<vector<int>>& a) {
    int c1 = 0, c2 = 0;
    for (int i = 0; i < 5; i++) {
        if (a[x][i] < a[y][i])
            c1++;

...
void solve() {
    ...
    sort(all(one), [&](auto &i, auto &j) { return w[i] > w[j]; });
    sort(all(two), [&](auto &i, auto &j) { return w[i] > w[j]; });
    // one + 2*two -> one = [0, one.size()], two = [0, two.size()];
    int ans = INF, s1 = 0, s2 = 0;
    for (auto &i : one) s1 += w[i];
    for (auto &i : two) s2 += w[i];
    int w1 = s1;
    for (int o = 0; o <= one.size(); o++) {
        int w2 = s2;
        for (int tw = 0; tw <= two.size(); tw++) {
            int topp = w1 + w2;
            int bott = o + 2 * tw;
            if (topp <= bott) {
                ans = min(ans, bott);

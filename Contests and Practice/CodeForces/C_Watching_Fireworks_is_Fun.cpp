...
const long long INFLL = 922337203685477580ll;
#pragma region
#include <algorithm>
#include <chrono>
#include <climits>
...
void solve() {
    ...
    vll DP(n);
    for (int i = 0; i < n; i++) {
        DP[i] = b[m - 1] - abs(a[m - 1] - i);
    }

    for (int ti = m - 2; ti >= 0; ti--) {
        ll t = T[ti + 1] - T[ti];
        t *= d;
        ll i = 0, r = 0;
        deque<pair<int, ll>> q;
        vll nDP(n);
        while (i < n) {
            ll cur = b[ti] - abs(a[ti] - i);
            while (r <= min(n - 1, i + t)) {
                while (q.size() && q.back().second <= DP[r]) {
                    q.pop_back();
                }
                q.push_back({r, DP[r]});
                r++;
            }
            while (q.size() && q.front().first < max(0ll, i - t)) {
                q.pop_front();
            }
            nDP[i] = cur + q.front().second;
            i++;
        }
        DP = nDP;
    }

...
void solve() {
    ...
    vector<vector<int>> vis(n, vector<int>(m, 5));
    auto dfs = [&](auto i, auto j, int cur, auto &dfs) -> bool {
        if (i < 0 || i >= n || j < 0 || j >= m || vis[i][j] == cur || a[i][j] == 'S' || a[i][j] == '#') return false;
        if (vis[i][j] < cur) return true;
        vis[i][j] = cur;
        for (auto &[dx, dy] : del) {
            if (dfs(i + dx, j + dy, cur, dfs))
                return true;
        }
        return false;
    };
    ...
    int cur = 0;
    for (auto &[dx, dy] : del) {
        if (dfs(st.ff + dx, st.ss + dy, cur, dfs)) {
            cout << "Yes\n";
            return;
        }
        cur++;

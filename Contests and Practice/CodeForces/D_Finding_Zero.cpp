...
int cc(int x, int y, int z) {
    multiset<int> st = {x, y, z};
    if (!CHK(m, st)) {
        cout << "? " << x << " " << y << " " << z << endl;
        int xx;
        cin >> xx;
...
pair<pii, pii> calc(vi a) {
    int mx = -1;
    map<int, int> m;
    fo(i, 4) {
        fok(j, i + 1, 4) {
            fok(k, j + 1, 4) {
                int x = cc(a[i], a[j], a[k]);
                if (x == mx) {
                    m[a[i]]++;
                    m[a[j]]++;
                    m[a[k]]++;
...
pii fun(int l, int r) {
    if (r - l + 1 >= 8) {
        ...
        auto [a, b] = fun(l, mid);
        auto [c, d] = fun(mid + 1, r);
        return calc({a, b, c, d}).ff;
    }
    if (r - l + 1 == 4)
        return calc({l, l + 1, l + 2, l + 3}).ff;
    auto p = calc({l, l + 1, l + 2, l + 3});
    int i = l + 4;
    pii cur = p.ff, ncur = p.ss;
    while (i + 1 <= r) {
        auto p2 = calc({ncur.ff, ncur.ss, i, i + 1}).ff;
        auto p3 = calc({cur.ff, cur.ss, p2.ff, p2.ss});
        cur = p3.ff;
        ncur = p3.ss;
        i += 2;
    }
    if (i == r) {
        auto p3 = calc({cur.ff, cur.ss, ncur.ff, i});
        cur = p3.ff;
    }
    return cur;
}

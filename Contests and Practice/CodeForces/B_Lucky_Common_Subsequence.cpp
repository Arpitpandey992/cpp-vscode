...
string dp(int i, int j, int k) {
    ...
    if (s1[i] == s2[j]) {
        if (s1[i] != v[k]) {
            ...
            if (a == 0 && s1[i] != v[0]) a--;
            string t = dp(i + 1, j + 1, a + 1);
            if (t != "x")
                return DP[i][j][k] = s1[i] + t;
            return DP[i][j][k] = t;
        }
        string t1 = dp(i + 1, j + 1, k);
        string t2 = dp(i + 1, j + 1, k + 1);
        if (t1 == "x" && t2 == "x")
            return DP[i][j][k] = t1;
        if (t1 == "x")
            return DP[i][j][k] = s1[i] + t2;
        if (t2 == "x")
            return DP[i][j][k] = t1;
    ...
    string t1 = dp(i + 1, j, k);
    string t2 = dp(i, j + 1, k);
    if (t1 == "x" && t2 == "x")
        return DP[i][j][k] = t1;
    if (t1 == "x")
        return DP[i][j][k] = t2;
...
void solve() {
    fo(i, N) fo(j, N) fo(k, N) DP[i][j][k] = "a";
    cin >> s1 >> s2 >> v;
    n = s1.size();
    m = s2.size();
    l = v.size();

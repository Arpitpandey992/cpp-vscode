...
bool coll(int &x, int &y, vpii &sn) {
    fok(i, 1, siz) if (x == sn[i].ff && y == sn[i].ss) return true;
    return false;
}
...
void shift(int x, int y, bool head, vpii &sn) {
    pii p = {x, y};
    if (head)
        fo(i, siz)
            swap(sn[i], p);
...
void solve() {
    ...
    while (q.size()) {
        ...
        pii head = sn[0], tail = sn[siz - 1];
        int ans = INF;
        fo(k, 4) {
            int x = head.ff + dx[k], y = head.ss + dy[k];
            shift(x, y, true, sn);
            if (x < 0 || x >= n || y < 0 || y >= m)
                ;
            else if (a[x][y] == '#' || coll(x, y, sn))
                ;
            else if (CHK(vis, sn))
                ;
            else {
            ...
            shift(tail.ff, tail.ss, false, sn);
        }
    }
    cout << -1 << endl;
}

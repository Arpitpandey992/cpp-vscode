...
int par[N][LOG + 1];
int fin(int x) {
    return par2[x] = ((par2[x] == x) ? x : fin(par2[x]));
}
void uni(int x, int y) {  // uni (x,y) -> x must be a parent of y
    int px = fin(x), py = fin(y);
    if (px == py) return;
    siz[px] += siz[py];
    par2[py] = px;
}
int lift(int i, int x) {
    for (int j = 30; j >= 0; j--) {
        if (x & (1 << j)) {
            i = par[i][j];
        }
...
void solve() {
    ...
    vi vis(n, 0);
    vpii cyc(n, {-1, -1});  // cyc==0? ->part of a cycle, otherwise -> connects to cycle after cyc[i] nodes;
    fo(i, n) {
        if (vis[i] == 2) continue;
        stack<int> s;
        s.push(i);
        while (s.size()) {
            ...
            if (vis[x] == 0) {
                s.push(x);
            } else if (vis[x] == 2) {  // visited before
                while (s.size()) {
                    auto tt = s.top();
                    vis[tt] = 2;
                    s.pop();
                    cyc[tt] = {cyc[x].ff + 1, cyc[x].ss};
                    x = tt;
                }
                break;
            } else {
                ...
                pii curNode = cyc[s.top()];
                s.pop();  // now only dangling links are present
                while (s.size()) {
                    cyc[s.top()] = {curNode.ff + 1, curNode.ss};
                    vis[s.top()] = 2;
                    curNode = cyc[s.top()];
                    s.pop();
                }
    ...
    vi id(n, -1);
    fo(i, n) {
        if (cyc[i].ff != 0 || len[i] != -1) continue;
        int cur = i;
        stack<int> s;
        s.push(cur);
        while (nex[cur] != i) {
    ...
    while (q--) {
        ...
        int ans = 0;
        int l1 = cyc[a].ff, l2 = cyc[b].ff;
        if (l2 > l1) {
            cout << -1 << endl;
            continue;
        }
        int potential = lift(a, l1 - l2);
        if (potential == b) {
            cout << l1 - l2 << endl;
            continue;
        }
        ...
        a = cyc[a].ss;
        if (id[a] < id[b]) {
            cout << ans + id[b] - id[a] << endl;
        } else {
            cout << ans + len[a] - (id[a] - id[b]) << endl;
        }
    }
}

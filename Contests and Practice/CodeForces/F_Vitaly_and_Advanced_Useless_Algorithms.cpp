...
// const int INF = 2147483647 / 100;
const long long INF = 9223372036854775807ll/100ll;
#pragma region
#include <algorithm>
#include <chrono>
#include <cmath>
...
ll dp(vector<tiii> &a, vector<ll> &ans) {  // time req, percentage possible, id
    ll c = 100, n = a.size();
    ll DP[n][c + 1];
    ll ch[n][c + 1];
    for (ll i = 0; i < n; i++) {
        auto [t, p, id] = a[i];
        for (ll perc = 0; perc <= 100; perc++) {
            if (perc == 0) {
                DP[i][perc] = 0;
                ch[i][perc] = 0;
                continue;
            ...
            if (i == 0) {
                DP[i][perc] = (perc <= p ? t : INF);
                ch[i][perc] = (perc <= p ? 1 : 0);
                continue;
            }
            if (DP[i - 1][perc] < t + DP[i - 1][max(0ll, perc - p)]) {
                ch[i][perc] = 0;
                DP[i][perc] = DP[i - 1][perc];
            } else {
                ch[i][perc] = 1;
                DP[i][perc] = t + DP[i - 1][max(0ll, perc - p)];
            }
        }
    }
    ll i = n - 1;

...
int n, m;
ll cnt = 0;
int fun(int pa, int pb, int ind) {
    if (dp[pa][pb][ind] != -1)
        return dp[pa][pb][ind];
    if (ind == m) {
        int ct = 1;
        ct = modmul(ct, pb - pa + 2);
        ct = modmul(ct, pb - pa + 1);
        ct /= 2;
        return dp[pa][pb][ind] = ct;
    }
    int ct = 0;
    for (int i = pb; i >= 1; i--) {
        for (int j = pa; j <= i; j++) {
            if (dp[j][i][ind + 1] == -1)
                dp[j][i][ind + 1] == fun(j, i, ind + 1);
            ct = modadd(ct, dp[j][i][ind + 1]);
        }
    }
    dp[pa][pb][ind] = ct;
    return ct;
...
int main() {
    fastio;
    cin >> n >> m;
    fo(i, n + 1) fo(j, n + 1) fo(k, m + 1) dp[i][j][k] = -1;
    cout << fun(1, n, 1) << endl;
}
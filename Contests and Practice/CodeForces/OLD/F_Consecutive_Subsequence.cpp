...
//--------------------------------------------------------------------//
vi DP[200001];
vector<map<int, vector<int>>> m;
vi a;
int n;
vi dp(int ind, int prev) {
    vi ans;
    if (ind == n) {
        return DP[ind];
    }
    if (a[ind] == prev + 1 || prev == -1) {
        if (DP[ind].empty()) {
            DP[ind].pb(ind + 1);
            vi temp = dp(ind + 1, a[ind]);
            if (!temp.empty())
                DP[ind].insert(DP[ind].end(), all(temp));
        }
        return DP[ind];
    } else {
        if (m[ind].find(prev) == m[ind].end())
            m[ind][prev] = dp(ind + 1, prev);
        return m[ind][prev];
    }
}
int main() {

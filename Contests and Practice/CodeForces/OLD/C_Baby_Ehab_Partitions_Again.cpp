...
ll sum = 0;
int DP[101][200001];
bool dp(int i, int sum, vi& a) {
    if (i == 0) {
        if (a[i] == sum || sum == 0) return true;
        return false;
    }
    if (sum < a[i]) return DP[i - 1][sum] = dp(i - 1, sum, a);
    if (sum == 0) return true;
    if (DP[i - 1][sum] == -1) DP[i - 1][sum] = dp(i - 1, sum, a);
    if (DP[i - 1][sum - a[i]] == -1) DP[i - 1][sum - a[i]] = dp(i - 1, sum - a[i], a);

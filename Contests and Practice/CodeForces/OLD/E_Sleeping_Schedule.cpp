...
ll DP(int hr, int i)
{
    if (i >= n)
        return 0;
    int newt = modadd(a[i], hr, h);
    int newt2 = modadd(a[i] - 1, hr, h);
    if (dp[newt][i + 1] == -1)
        dp[newt][i + 1] = DP(newt, i + 1);

    if (dp[newt2][i + 1] == -1)
        dp[newt2][i + 1] = DP(newt2, i + 1);

    if (dp[newt][i + 1] > dp[newt2][i + 1])
    {
        if (newt >= l && newt <= r)
            return 1 + dp[newt][i + 1];
        else
            return dp[newt][i + 1];
    }
    else if (dp[newt][i + 1] < dp[newt2][i + 1])
    {
        if (newt2 >= l && newt2 <= r)
            return 1 + dp[newt2][i + 1];
        else
            return dp[newt2][i + 1];
    }
    else
    {
        if (newt2 >= l && newt2 <= r)
            return 1 + dp[newt2][i + 1];
        else if (newt >= l && newt <= r)
            return dp[newt][i + 1] + 1;
        return dp[newt][i + 1];
    }
}

...
int dnc(string &s, int i, int j, char c) {
    if (j - i + 1 == 1)
        return (s[i] == c ? 0 : 1);
    int ct1 = 0, ct2 = 0;
    int ans1 = dnc(s, (j + i + 1) / 2, j, c + 1);
    int ans2 = dnc(s, i, ((j + i + 1) / 2) - 1, c + 1);
    for (int k = i; k < (j + i + 1) / 2; k++) {
        if (s[k] != c)
            ct1++;
    }
    for (int k = (j + i + 1) / 2; k < j + 1; k++) {
        if (s[k] != c)
            ct2++;
    }
    return min(ct1 + ans1, ct2 + ans2);
}

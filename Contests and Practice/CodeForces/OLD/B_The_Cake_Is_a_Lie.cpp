...
//int DP[105][105][10500];
bool dp(int i, int j, int cur) {
    if (i == n && j == m && cur == k) return 1;
    if (i > n || j > m || cur > k) return 0;
    if (DP[i + 1][j].find(cur + j) == DP[i + 1][j].end())
        DP[i + 1][j][cur + j] = dp(i + 1, j, cur + j);
    if (DP[i][j + 1].find(cur + i) == DP[i][j + 1].end())
        DP[i][j + 1][cur + i] = dp(i, j + 1, cur + i);

    return DP[i + 1][j][cur + j] | DP[i][j + 1][cur + i];
}

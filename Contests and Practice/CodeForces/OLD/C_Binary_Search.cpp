...
int main() {
    ...
    less--;
    ll rest = (n - less - more - 1);
    //ans=(x-1)Cless*(n-x)Cmore+rest!;
    ll ans = 1;
    ll factl = 1, factm = 1;
    for (ll i = less; i >= 1; i--) {
        factl = modmul(factl, i, M);
    }
    for (ll i = more; i >= 1; i--) {
        factm = modmul(factm, i, M);
    }
    ans = modmul(ans, (nCrModPFermat(x - 1, less, M)), M);
    ans = modmul(ans, (nCrModPFermat(n - x, more, M)), M);
    ans = modmul(ans, factl, M);
    ans = modmul(ans, factm, M);
    for (ll i = rest; i >= 1; i--) {
        ans = modmul(ans, i, M);
    }
    cout << ans << endl;

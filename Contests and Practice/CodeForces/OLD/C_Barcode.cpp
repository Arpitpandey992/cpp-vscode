...
int dp(int i, int z, int ch) {
    if (z > y) return INF;
    if (i < 0) {
        return z >= x ? 0 : INF;
    }
    if (DP[i][z][ch] != -1) return DP[i][z][ch];
    if (z < x) return DP[i][z][ch] = cost[i][ch] + dp(i - 1, z + 1, ch);
    return DP[i][z][ch] = min(dp(i - 1, z + 1, ch) + cost[i][ch], dp(i - 1, 1, !ch) + cost[i][!ch]);
}
//0=white, 1=black
int main() {
    fastio;
    cin >> n >> m >> x >> y;
    fo(i, m) fo(j, m) fo(k, 2) DP[i][j][k] = -1;
    fo(i, n) fo(j, m) cin >> bar[i][j];
    fo(j, m) {
        int b = 0, w = 0;
        fo(i, n) {
            if (bar[i][j] == '.')
    ...
    cout << min(cost[m - 1][0] + dp(m - 2, 1, 0), cost[m - 1][1] + dp(m - 2, 1, 1)) << endl;
}
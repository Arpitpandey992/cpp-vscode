...
ll dfs(int x, int y, vs &a, vvb &vis) {
    if (x < 0 || x >= n || y < 0 || y >= m || vis[x][y] || a[x][y] == '.') return 0;
    vis[x][y] = true;
    int ans = 1;
    fo(k, 8) {
        ans += dfs(x + dx[k], y + dy[k], a, vis);
    }
    return ans;
}
...
void solve() {
    ...
    fo(i, n) {
        fo(j, m) {
            ...
            int ct = dfs(i, j, a, vis);
            if (ct != 3) {
                cout << "NO\n";
                return;
            }
            int c = 0;
            for (int k = 0; i + k < min(n, i + 2); k++) {
                for (int l = 0; j + l < min(m, j + 2); l++) {
                    if (a[i + k][j + l] == '*') c++;
                }
            }
            if (c != 3) {
                c = 0;
                for (int k = 0; i + k < min(n, i + 2); k++) {
                    for (int l = -1; j + l < min(m, j + 1) && j + l >= 0; l++) {
                        if (a[i + k][j + l] == '*') c++;
                    }
                }
                if (c != 3) {

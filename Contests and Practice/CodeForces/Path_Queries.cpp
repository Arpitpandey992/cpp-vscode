...
void dfs(ll i, vector<ll> adj[], vll &path, int par, vll &a, vector<pll> &occ) {
    path.pb(a[i]);
    occ[i].first = path.size() - 1;
    for (auto &x : adj[i]) {
        if (x != par)
            dfs(x, adj, path, i, a, occ);
    }
    path.pb(-a[i]);
    occ[i].second = path.size() - 1;
}
...
void solve() {
    ...
    vll path;
    vector<pair<ll, ll>> occ(n);
    dfs(0, adj, path, -1, a, occ);
    ll siz = 1 << int(ceil(log2(path.size())));
    vll st(2 * siz, 0);

    for (ll i = siz; i - siz < path.size(); i++)
        st[i] = path[i - siz];
        
    for (ll i = siz - 1; i > 0; i--)
        st[i] = st[i * 2] + st[i * 2 + 1];

    auto upd = [&st, &siz](ll i, ll x) {
        i += siz;
        st[i] = x;
        i /= 2;
        while (i > 0) {

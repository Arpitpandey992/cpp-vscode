...
ll dp(ll i, ll j, ll len) {
    if (i == n) return 0;
    if (j == -1) {
        ll ans = dp(i + 1, j, len);
        for (ll k = i; k < n; k++)
            ans = max(ans, cost[k - i] + dp(k + 1, i, k - i + 1));
        return ans;
    }
    if (s[i] < s[j]) return dp(i + 1, j, len);
    if (CHK(DP, make_tuple(i, j, len)))
        return DP[make_tuple(i, j, len)];
    ll ans = 0;
    for (ll k = i; k < n; k++) {
        if (s.substr(j, len) < s.substr(i, k - i + 1))
            ans = max(ans, cost[k - i] + dp(k + 1, i, k - i + 1));
    }
    return DP[make_tuple(i, j, len)] = ans;
}

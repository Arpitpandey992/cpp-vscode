...
Mint getValue(vector<Mint> &ct, int n) {
    int k = ct.size();
    vector<Mint> DP(k, 0);
    DP[0] = 1;
    for (int i = n - 1; i >= 0; i--) {
        ...
        for (int j = 0; j < k; j++) {
            for (int choose = 0; choose < k; choose++) {
                nDP[j] += ct[choose] * DP[((j - choose) % k + k) % k];
            }
        }
        DP = nDP;
    }
...
Mint getVal(vector<Mint> &ct, int n, int k) {
    Mint perc = getValue(ct, n % k);
    Mint whole = getValue(ct, k);
    int ctt = n / k;
    return perc * (whole ^ ctt);
}

void solve() {
    int n, p, k;
    cin >> n >> p >> k;
    vector<Mint> ct(k, floor(p, k));
    for (int i = 1; i <= p % k; i++)
        ct[i]++;
    Mint all = getVal(ct, n, k);
    int m;
    cin >> m;
    fo(i, m) {
        int x;
    ...
    Mint without = getVal(ct, n, k);
    cout << all - without << endl;
}

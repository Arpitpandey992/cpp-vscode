...
bool can(int lef, int rig, int roundLeft) {
    if (rig < 0 || lef < 0) return false;
    if (roundLeft == 0) return rig == 0;
    // pair every rig with lef if possible
    int pairable = min(lef, rig);
    if ((lef - pairable) % 2 != 0 || (rig - pairable) % 2 != 0)
        return false;
    return can(pairable + (lef - pairable) / 2, (rig - pairable) / 2, roundLeft - 1);
}

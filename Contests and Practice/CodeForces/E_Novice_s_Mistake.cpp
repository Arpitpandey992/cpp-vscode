...
void solve() {
    ...
    for (int a = 2; a <= 10000; a++) {
        for (int b = strn.size() * a - 1; strn.size() * a - b <= to_string(n * a - b).size(); b--) {
            int siz = strn.size() * a - b;
            string cur = "";
            while (cur.size() + strn.size() < siz)
                cur += strn;
            cur += strn.substr(0, siz - cur.size());
            if (cur != "" && stoi(cur) == n * a - b)
                ans.pb({a, b});
        }
    }
    cout << ans.size() << endl;

...
//-------------------------------------------------------------------------------//
vector<int> dep;
vector<vector<int>> adj, nodesAtDepth, depthTimes, par;
vpii tim;

void dfs(int i, int &curTime, int curDep) {
    tim[i].ff = curTime;
    dep[i] = curDep;
    curTime++;
    for (auto &x : adj[i])
        dfs(x, curTime, curDep + 1);
    tim[i].ss = curTime;
}
void bfs(int i) {
    queue<int> q;
    ...
    while (q.size()) {
        ...
        nodesAtDepth.push_back(cur);
    }
    for (auto &nodes : nodesAtDepth) {
        vector<int> cur;
        for (auto &node : nodes) {
            cur.pb(tim[node].ff);
        }
...
int getNodeAtHeight(int i, int h) {
...
void solve() {
    ...
    cin >> n;
    n++;
    int LOG = ceil(log2(n + 1)) + 1;
    dep.resize(n);
    tim.resize(n, {-1, -1});
    adj.resize(n);
    par.resize(n, vector<int>(LOG + 1, -1));
    fok(i, 1, n) {
        int x;
        cin >> x;
        adj[x].pb(i);
    ...
    while (queries--) {
        int v, p;
        cin >> v >> p;
        int node = getNodeAtHeight(v, p);
        if (node == -1 || node == 0) {
            cout << 0 << " ";
            continue;
        }
        int nodeDepth = dep[node];
        int reqDepth = nodeDepth + p;
        vector<int> &nodes = depthTimes[reqDepth];
        int st = tim[node].ff, ed = tim[node].ss;
        int ans = ub(nodes, ed) - ub(nodes, st);
        cout << ans - 1 << " ";
    }
    cout << endl;
}

...
ll n, c;
ll dp(ll i, vector<ll> adj[], ll parCol, ll cant, ll par) {
    ll ans = max(c - 2, parCol - 1);
    ll ch = max(0ll, max(c - 2, parCol - 1) - cant);
    ll cur = 0;
    for (auto &x : adj[i]) {
        if (x != par) {
            ans = (ans * dp(x, adj, ch, cur, i) % M);
            cur++;
        }
    }
    return ans;

...
vi cnt(N, 0);
vi flat;
int lef = -1, rig = -1, ans = 0;
vector<pair<int, int>> pos(N);
vi st(4 * N + 5, 0);
void upd(int i, int k, int sl, int sr, int node) {
    if (i < sl || i > sr) return;
    st[node] += k;
    if (sl == sr) return;
    int mid = midpoint(sl, sr);
    upd(i, k, sl, mid, node * 2 + 1);
    upd(i, k, mid + 1, sr, node * 2 + 2);
}
int qry(int l, int r, int sl, int sr, int node) {
    if (r < sl || sr < l) return 0;
    if (sl >= l && sr <= r) return st[node];
    int mid = midpoint(sl, sr);
    return qry(l, r, sl, mid, node * 2 + 1) +
           qry(l, r, mid + 1, sr, node * 2 + 2);
}
void dfs(int i, int par) {
    flat.pb(i);
    pos[i].ff = flat.size() - 1;
    for (auto x : adj[i]) {
        if (x != par)
            dfs(x, i);
    }
    // flat.pb(i);
    pos[i].ss = flat.size() - 1;
}

void add(int i) {
    if (i < 0 || i >= n) return;
    auto &cc = cnt[c[flat[i]]];
    if (cc != 0)
        upd(cc, -1, 0, N - 1, 0);
    cc++;
    upd(cc, 1, 0, N - 1, 0);
...
void solve() {
    ...
    fo(i, m) {
        int v, k;
        cin >> v >> k;
        q.pb({pos[v - 1].ff, pos[v - 1].ss, k, i});
    }
    int buck_siz = sqrt(m);
    sort(all(q), [&buck_siz](auto &t1, auto &t2) {
        auto &[l1, r1, k1, i1] = t1;
        auto &[l2, r2, k2, i2] = t1;
        if ((l1 / buck_siz) < (l2 / buck_siz)) return true;
        if ((l1 / buck_siz) > (l2 / buck_siz)) return false;
        return r1 < r2;
    });

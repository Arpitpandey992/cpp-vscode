...
ll query(vvll &st, ll l, ll n) {
    ll m = msd(n);
    if (n - (1 << m) == 0)
        return st[l][m];
    return __gcd(st[l][m], query(st, l + (1 << m), n - (1 << m)));
}
...
void solve() {
    ...
    fo(i, n) cin >> a[i];
    ll maxp = (ceil(log2(n)));
    vvll st(n, vll(maxp + 1));
    for (int siz = 0; siz <= maxp; siz++) {
        for (int i = 0; i + (1 << siz) <= n; i++) {
            if (siz == 0) {
                st[i][siz] = a[i];
                continue;
            }
            st[i][siz] = __gcd(st[i][siz - 1], st[i + (1 << (siz - 1))][siz - 1]);
        }
    }
    // while (1) {
    //     int l, r;
    //     cin >> l >> r;
    //     cout << query(st, l, r - l + 1);
    // }
    ll prev = -1, cur = 0;
    fo(i, n) {
        ll l = 1, r = i + 1, mxlen = i - prev;
        bool flag = false;
        while (l <= r) {
            ll mid = midpoint(l, r);
            ll lef = i - mid + 1;
            ll gc = query(st, lef, mid);
            if (l == r) {
                if (gc == mid && mid <= mxlen) flag = true;
                break;
            }
            if (gc > mid) {
                l = mid + 1;

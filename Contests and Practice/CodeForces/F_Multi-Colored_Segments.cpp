...
void solve() {
    ...
    for (auto &[l, r, c, id] : a) {
        if (s.size()) {
            auto &[r1, c1, i1] = *s.rbegin();
            if (c1 != c) {
                ans[id] = min(ans[id], max(0, l - r1));
                ans[i1] = min(ans[i1], max(0, l - r1));
            } else if (s.size() > 1) {
                auto &[r2, c2, i2] = *(++s.rbegin());
                ans[id] = min(ans[id], max(0, l - r2));
                ans[i2] = min(ans[i2], max(0, l - r2));
            }
        }
        if (!m.contains(c)) {
            m[c] = {r, id};

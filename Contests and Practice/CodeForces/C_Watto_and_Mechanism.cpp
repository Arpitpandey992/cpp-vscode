...
class Trie {
    ...
    bool src(string& s, int i, node* cur, bool used) {
        if (i == s.size()) return cur->ect && used;
        int c = s[i] - 'a';
        if (cur->next[c] && src(s, i + 1, cur->next[c], used))
            return true;
        if (used) return false;
        for (int j = 0; j < 3; j++) {
            if (j != c && cur->next[j] && src(s, i + 1, cur->next[j], true))
                return true;
        }
        return false;
    }

...
bool getCt(vector<int> &a, int l, int r, int k) {
    while (l <= r) {
        int mid = (l + r) >> 1;
...
void solve() {
    ...
    fo(i, siz) {
        pref[i] = cyc[i] + (i > 0 ? pref[i - 1] : 0);
    }

    for (int k = 1; k <= n; k++) {
        if (getCt(cyc, 0, siz - 1, k)) {
            cout << 0 << " ";
            continue;
        }
        if (cyc[0] > k) {
        ...
        for (int j = 0; j <= i; j++) {
            ...
            bool there = getCt(cyc, i + 1, siz - 1, req);
            if (there) {
                ans = i;
                break;
            }

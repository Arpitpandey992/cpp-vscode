...
pair<ll, ll> dp(ll x, ll y, ll boat) {
    if (x == 0 && y == 0) return {0, 1};
    if (DP[x][y][boat].first == -2) return {INF / 2, 0};
    if (DP[x][y][boat].first != -1) return DP[x][y][boat];
    DP[x][y][boat] = {-2, -2};
    ll mainMoves = INF / 2;
    ll mainWays = 0;
    if (boat) {
        for (ll i = 0; i <= x; i++) {
            for (ll j = 0; j <= y; j++) {
                ll w = j * 100 + i * 50;
                if (w == 0 || w > k) continue;
                auto [moves, ways] = dp(x - i, y - j, !boat);
                ways = ((ways * NCR[x][i]) % M * NCR[y][j]) % M;
                if (moves + 1 < mainMoves) {
                    mainMoves = moves + 1;
                    mainWays = ways;
                } else if (moves + 1 == mainMoves)
                    mainWays = (mainWays + ways) % M;
            }
        }
    } else {
        for (ll i = 0; i <= n1 - x; i++) {
            for (ll j = 0; j <= n2 - y; j++) {
                ...
                if (w == 0 || w > k) continue;
                auto [moves, ways] = dp(x + i, y + j, !boat);
                ways = ((ways * NCR[n1 - x][i]) % M * NCR[n2 - y][j]) % M;
                if (moves + 1 < mainMoves) {
                    mainMoves = moves + 1;
                    mainWays = ways;
                } else if (moves + 1 == mainMoves)
    ...
    return DP[x][y][boat] = {mainMoves, mainWays};
}
...
void solve() {
    ...
    fo(i, n1 + 1) fo(j, n2 + 1) DP[i][j][0] = DP[i][j][1] = {-1, -1};
    auto [moves, ways] = dp(n1, n2, 1);
    for (int i = 0; i <= n1; i++) {
        for (int j = 0; j <= n2; j++) {
            cout << i << " " << j << "->" << DP[i][j][0].second << endl;
        }
        nl;
    }
    nl;
...
// DP[i][j][0] -> number of ways to transport if we have i 50 kg and j 100kg persons on left side
// DP[i][j][1] -> boat is at right side -> some people will have to get it to left side and resume
// DP[i][j][0] = DP[i-1][j][1] + DP[i-2][j][1]+...+DP[i][j-1][1]+... all combinations
//

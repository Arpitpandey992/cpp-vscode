...
pll query(int l, int r, int sl, int sr, int node) {
    if (sl > r || sr < l)
        return {1, 0};
    if (sl <= r && sr >= l)
        return st[node];
    int mid = (sl + sr) / 2;
    auto lef = query(l, r, sl, mid, node * 2 + 1);
    auto rig = query(l, r, mid + 1, sr, node * 2 + 2);
    return {(lef.ff * rig.ff) % M, (lef.ff + rig.ss)};
}

ll update1(int l, int sl, int sr, int node, ll num, ll den) {
    if (sr < l || sl > l)
        return 1;
    if (sl == sr)
        return st[node].ff = moddiv(modmul(st[node].ff, num), den);
    int mid = (sl + sr) / 2;
    auto lef = update1(l, sl, mid, node * 2 + 1, num, den);
    auto rig = update1(l, mid + 1, sr, node * 2 + 2, num, den);
    return st[node].ff = (lef * rig) % M;
}
ll update2(int l, int sl, int sr, int node) {
    if (sr < l || sl > l)
        return 0;
    if (sl == sr)
        return st[node].ss = st[node].ss + 1;
    int mid = (sl + sr) / 2;
    auto lef = update2(l, sl, mid, node * 2 + 1);
    auto rig = update2(l, mid + 1, sr, node * 2 + 2);
    return st[node].ss = (lef + rig) % M;
}
void solve() {
    int n;
    cin >> n;
    ...
    fo(i, n) {
        ...
        pll p = {1, 0};
        if (x - 2 >= 0)
            p = query(0, x - 2, 0, MAXN - 1, 0);
        ll nleft = i + 1 - p.ss;
        ll po2 = modexp(2, nleft);
        ans = modadd(ans, modmul(po2, p.ff));
        for (int i = x; i < MAXN;i++){
            
            update1(i, 0, MAXN - 1, 0, num, den);
        }
    }
}

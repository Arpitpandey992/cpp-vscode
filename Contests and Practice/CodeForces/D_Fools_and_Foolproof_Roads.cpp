...
ll mx = 1e9;
ll fin(ll i) {
    return par[i] = (par[i] == i ? i : fin(par[i]));
}
void uni(ll x, ll y) {
...
void solve() {
    ...
    vector<pll> ans;
    while (p > 0) {
        if (Q.size() == q || Q.size() <= 1) break;
        auto [s1, x1] = Q.top();
        Q.pop();
        auto [s2, x2] = Q.top();
        Q.pop();
        ans.pb({fin(x1), fin(x2)});
        uni(x1, x2);
        Q.push({siz[fin(x1)], fin(x1)});
        p--;
    }
    if (p > 0) {
        ll u = -1, v = -1;

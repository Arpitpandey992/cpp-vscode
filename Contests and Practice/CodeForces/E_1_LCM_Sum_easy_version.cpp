...
ll getBetween(ll l, ll x) {
    auto it = lb(f[x], l) - f[x].begin();
    ll factors = f[x].size() - it;
    if (factors - 1 < 2)
        return 0;
    return ((factors - 1) * (factors - 2)) / 2;
}
ll get432(ll l, ll x) {
    ll num = x * 2;
    if (num % 3 != 0 || num % 4 != 0)
        return 0;
    ll n1 = num / 3, n2 = num / 4;
    if (n1 < l || n2 < l)
        return 0;
    return 1;
}
ll get532(ll l, ll x) {
    ...
    if (num % 3 != 0 || num % 5 != 0)
        return 0;
    ll n1 = num / 3, n2 = num / 5;
    if (n1 < l || n2 < l)
        return 0;
    return 1;
}
ll getAllLess(ll l, ll x) {
    return getBetween(l, x) + get432(l, x) + get532(l, x);
}
ll getTriplets(ll l, ll r) {
    ll ans = 0;
    for (int i = l + 2; i <= r; i++) {
        ans += getAllLess(l, i);
    }
    ll len = r - l + 1;
    ll AllTriplets = (len * (len - 1) * (len - 2)) / 6;
    return max(0ll, AllTriplets - ans);
}
void solve() {
    int l, r;
    cin >> l >> r;
    cout << getTriplets(l, r) << endl;
}

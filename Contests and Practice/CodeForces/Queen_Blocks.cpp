...
bool chk(int x, int y, vector<vector<int>> &a) {
    if (x < 0 || x >= 8 || y < 0 || y >= 8) return true;
    for (int i = 0; i < 8; i++)
        if (a[i][y] == 2) return true;
    for (int j = 0; j < 8; j++)
        if (a[x][j] == 2) return true;
    int i = x, j = y;
    while (i >= 0 && j >= 0) {
...
bool chk1(int x, int y, vector<vector<int>> &a) {
    if (x < 0 || x >= 8 || y < 0 || y >= 8) return false;
    for (int i = 0; i < 8; i++)
        if (a[i][y] == 1) return true;
    for (int j = 0; j < 8; j++)
        if (a[x][j] == 1) return true;
    int i = x, j = y;
    while (i >= 0 && j >= 0) {
...
void solve() {
    ...
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; j++) {
            bool flag = true;
            if (a[i][j] == 1 || chk1(i, j, a)) continue;
            a[i][j] = 2;
            fo(k, 8) {
                if (!chk(x + dx[k], y + dy[k], a)) {
                    flag = false;
                    break;
                }
            }
    ...
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; j++) {
            for (int k = 0; k < n; k++) {
                for (int l = 0; l < n; l++) {
                    if (a[i][j] == 1 || a[k][l] == 1 || i == k && j == l || chk1(i, j, a) || chk1(k, l, a)) continue;
                    bool flag = true;
                    a[i][j] = 2;
                    a[k][l] = 2;
                    fo(d, 8) {
                        if (!chk(x + dx[d], y + dy[d], a)) {
                            flag = false;
                            break;
                        }
                    }

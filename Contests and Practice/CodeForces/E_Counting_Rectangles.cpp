...
void solve() {
    ...
    vvll m(mxw + 10, vll(mxh + 10, 0));
    for (auto &p : a) m[p.ff][p.ss] += p.ff * p.ss;
    ll DP[mxw + 10][mxh + 10];
    // width, height
    for (ll i = 0; i <= mxw + 1; i++) {
        for (ll j = 0; j <= mxh + 1; j++) {
            if (i == 0 || j == 0) {
                DP[i][j] = 0;
                continue;
            }
            DP[i][j] = m[i][j];
            DP[i][j] = max(0ll, DP[i][j] + DP[i - 1][j] + DP[i][j - 1] - DP[i - 1][j - 1]);
        }
    }
    while (q--) {
        ll hs, ws, hb, wb;
        ...
        hb--;
        wb--;
        cout << DP[wb][hb] - DP[ws][hb] - DP[wb][hs] + DP[ws][hs] << endl;
    }
}

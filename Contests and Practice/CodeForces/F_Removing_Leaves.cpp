...
void solve() {
    ...
    for (auto &[k, v] : ct) q.push({v, k});
    int ans = 0;
    while (q.size() && q.top().first >= k) {
        auto [howMany, who] = q.top();
        q.pop();
        if (ct[who] != howMany) continue;
        int increase = floor(howMany, k);
        ans += increase;
        auto &v = adj[who];
        int left = k * increase;
        auto it = v.begin();
        while (it != v.end() && left > 0) {
            if (in[*it] == 1) {  // leaf node -> remove
                left--;
                in[who]--;
                in[*it]--;
                adj[*(it)].erase(who);
        ...
        howMany %= k;
        if (howMany == 0 && adj[who].size() == 1) {  // this node became a leaf
            int par = *(adj[who].begin());
            if (adj[par].size() != 1) {
                ct[par]++;
                q.push({ct[par], par});
            }

...
int dp(int node, vector<pii> &where, vector<vector<pii>> &par) {
    if (node == 0) return 0;
    int ans = INF;
    for (auto &[who, w] : par[node]) {
        int val = dp(who, where, par) + !w;
        if (val < ans) {
            where[node] = {who, w};
            ans = val;
        }
...
//         if (vis[j] || bs(cant[node], j)) continue;
//         if (w == 1) {
//             ans.pb({node + 1, j + 1, !w});
//         }
//         dfs(j, ans, cant, adj, vis);
//     }
// }
...
void solve() {
    ...
    set<tuple<int, int, int>> ed;

    auto getShortestPathGraph = [&](int st, int ed) {
        vector<vector<pii>> par(n);
        queue<pair<int, int>> q;
        q.push({st, 0});
        vector<int> vis(n, INF / 2);
        vis[0] = 0;
        while (q.size()) {
    ...
    auto bestPath = getShortestPathGraph(0, n - 1);
    vector<tuple<int, int, int>> ans;
    int cur = n - 1;
    // vector<vector<int>> cant(n);
    while (cur != 0) {
        auto [nextNode, on] = bestPath[cur];
        if (!on)
            ans.pb({nextNode + 1, cur + 1, !on});
        ed.erase(make_tuple(min(cur, nextNode), max(cur, nextNode), on));
        // cant[cur].pb(nextNode);
        // cant[nextNode].pb(cur);
        cur = nextNode;
    }
